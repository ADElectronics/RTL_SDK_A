...
//======================================================
_LONG_CALL_ extern uint8_t UartLogCmdChk(
IN uint8_t RevData, IN UART_LOG_CTL *prvUartLogCtl,
IN uint8_t EchoFlag);
		...
		if (p->EscSTS == 2) {
			...
			if (UartLogCmdChk(UartReceiveData, (UART_LOG_CTL *) pUartLogCtl, 1)
				...
				if (p->pTmpLogBuf != NULL) {
					...
					ArrayInitialize((uint8_t *) pUartLogCtl->pTmpLogBuf->UARTLogBuf,
...
//======================================================
int GetArgvRam(IN uint8_t *pstr, uint8_t** argv) {
	int arvc = 0;
//	uint8_t** argv = ArgvArray;
	uint8_t* p = pstr;
	uint8_t t = 0, n = ' ';
	int m = 0;
	while(*p != 0
...
MON_RAM_TEXT_SECTION void console_init(void) {
	...
	UartIrqHandle.Data = 0; // (uint32_t)&UartAdapter;
