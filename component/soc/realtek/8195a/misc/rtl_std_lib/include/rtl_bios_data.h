...
extern uint32_t UserIrqDataTable[64];		// 10000200

/* ROM + diag.h */
extern uint32_t CfgSysDebugWarn;		// 10000300
extern uint32_t CfgSysDebugInfo;		// 10000304
extern uint32_t CfgSysDebugErr;		// 10000308
extern uint32_t ConfigDebugWarn;		// 1000030c
extern uint32_t ConfigDebugInfo;		// 10000310
extern uint32_t ConfigDebugErr;		// 10000314


/* ROM + hal_timer.h & .. */
extern HAL_TIMER_OP HalTimerOp; 		// 10000318
extern uint16_t GPIOState[11];				// 10000334
extern uint32_t gTimerRecord;				// 1000034C
/* ROM + hal_ssi.h */
extern uint32_t SSI_DBG_CONFIG;				// 10000350
...
/* ROM + Rand() */
extern uint32_t _rand_z4, _rand_z3, _rand_z2, _rand_z1, _rand_first; // 10000370..
...
extern uint8_t UartLogHistoryBuf[UART_LOG_HISTORY_LEN][UART_LOG_CMD_BUFLEN]; // 10000430 UartLogHistoryBuf[5][127] !
extern uint8_t *ArgvArray[MAX_ARGV]; 	// 100006AC *ArgvArray[10] !
...
typedef struct _CFO_TRACKING_ {
	...
	uint8_t DefXCap;
	uint32_t CFO_tail[2];
	uint32_t CFO_ave_pre;
	uint32_t packetCount;
	uint32_t packetCount_pre;
	BOOL bForceXtalCap;
	BOOL bReset;
	uint8_t CFO_TH_XTAL_HIGH;
	uint8_t CFO_TH_XTAL_LOW;
	uint8_t CFO_TH_ATC;
}CFO_TRACKING;
...
extern uint32_t __rtl_malloc_trim_threshold; 	// 10000b88  __rom_mallocr_init_v1_00()
extern uint32_t __rtl_malloc_top_pad; 			// 10000b8c  __rom_mallocr_init_v1_00()
extern uint8_t * __rtl_malloc_sbrk_base; 		// 10000b90 __rom_mallocr_init_v1_00()
extern uint32_t __rtl_malloc_max_sbrked_mem;		// 10000b94 __rom_mallocr_init_v1_00()
extern uint32_t __rtl_malloc_max_total_mem;		// 10000b98 __rom_mallocr_init_v1_00()
struct mallinfo
{
  int arena;
  int ordblks;
...
extern uint32_t rand_x; 							// 10000be4: ChangeRandSeed_rom(), Sdr_Rand2_rom()
#define REC_NUM 512
extern uint32_t AvaWds[2][REC_NUM];				// 10000be8
...
/* ROM ? UserData? */
extern uint32_t _rom_unc_data[9];				// 100020e8

/* ROM + hal_sdr_controller.c: Sdr_Rand2() */
extern uint32_t _sdr_rnd2_c, _sdr_rnd2_z, _sdr_rnd2_y; // 100020BC, 100020B8, 100020B4
