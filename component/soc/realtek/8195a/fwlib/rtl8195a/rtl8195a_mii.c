...
volatile uint8_t *pTxDataBuf = NULL;
volatile uint8_t *pRxDataBuf = NULL;

#if 0 
volatile TX_DESC_FMT TxDesc[MII_TX_DESC_NO];
volatile RX_DESC_FMT RxDesc[MII_RX_DESC_NO];
volatile uint32_t TxDescIdx = 0;
volatile uint32_t TxBufIdx = 0;
volatile uint32_t RxDescIdx = 0;
volatile uint32_t RxBufIdx = 0;
volatile uint8_t isTxDone = 0;
volatile uint8_t isRxDone = 0;
volatile uint32_t CurrDataLen = 0;
volatile uint32_t ReadDescIdx = 0;
volatile uint32_t ReadBufIdx = 0;
volatile uint32_t CurrSentSize = 0;
volatile uint32_t CurrReceivedSize = 0;
#endif
...
volatile uint8_t TxDescWrPtr = 0;
volatile uint8_t RxDescRdPtr = 0;

volatile uint32_t CurrDataLen = 0;
#if 0
HAL_GDMA_OBJ gdmaObj;
uint8_t isGdmaAllocated;
volatile uint8_t isDmaDone;
#endif
...
)
{
    ...
    volatile uint32_t RegValue = HAL_MII_READ32(REG_MII_ISRIMR);
...
uint16_t
HalMiiRwPhyRegRtl8195a(uint8_t Operation, uint8_t Address, uint16_t Data)
{
	uint32_t tmp;
	...
	else
	{
		...
		return (uint16_t)(HAL_MII_READ32(REG_MII_MIIA) & 0xFFFF);
	}
}
...
)
{
	...
	pMiiIrqHandle->Data	 = (uint32_t) pEthAdapter;
...
)
{
	...
	MiiIrqHandle.Data	 = (uint32_t) pEthAdapter;
...
#if 0
void Mii_DmaDone_ISR(uint32_t id)
{
    isDmaDone = 1;
}
#endif
...
)
{
	...
	HAL_MII_WRITE32(REG_MII_TXFDP1, (uint32_t)TxDesc);
	HAL_MII_WRITE32(REG_MII_RXFDP1, (uint32_t)RxDesc);
	...
	gdmaObj.GdmaIrqHandle.Data = (uint32_t)(&gdmaObj);
	...
	for(i = 0; i < (pEthAdapter->tx_desc_num); i++)
	{
		...
		TxDesc[i].addr = (uint32_t)(pTxDataBuf + (i * MII_BUF_SIZE));
	...
	{	
		...
		RxDesc[i].addr = (uint32_t)(pRxDataBuf + (i * MII_BUF_SIZE));
		RxDesc[i].dw2 = 0;
		RxDesc[i].dw3 = 0;
	}
...
)
{
	...
	uint8_t tx_serach_idx = TxDescWrPtr;
	if((((volatile uint32_t)TxDesc[tx_serach_idx].dw1) & TX_DESC_OWN) == 0)
	{
		#if 1
...
{
	uint8_t rx_serach_idx = RxDescRdPtr;
	uint32_t rx_len = 0;


	/* Check if Rx descriptor is available */
	if((((volatile uint32_t)RxDesc[rx_serach_idx].dw1) & RX_DESC_OWN) == 0)
	{
...
)
{
	...
	//RxDesc[read_idx].addr = (uint32_t)(pRxDataBuf + (read_idx * MII_BUF_SIZE));
...
void MiiIrqHandle (IN void *Data) {
    uint32_t RegValue = HalMiiGmacGetInterruptStatusRtl8195a();
	extern volatile uint8_t isRxOK;
	extern volatile uint8_t isTxOK;
	extern volatile uint8_t RxIntCnt;
        ...
        )
{
    ...
    pTx_Info->addr     = (uint32_t)TxBuffer;
        ...
        )
{
    ...
    pRx_Info->addr     = (uint32_t)RxBuffer;
		...
		)
{
	...
	MiiIrqHandle_Master.Data	 = (uint32_t) (pMiiAdapter);
