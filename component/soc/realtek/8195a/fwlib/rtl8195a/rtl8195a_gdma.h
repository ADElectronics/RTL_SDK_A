...
typedef struct _GDMA_CTL_REG_ {
        ...
        uint8_t                      IntEn           :1; // Bit 0
        uint8_t                      SrcGatherEn     :1; // Bit 1
        uint8_t                      DstScatterEn    :1; // Bit 2
        uint8_t                      LlpDstEn        :1; // Bit 3
        uint8_t                      LlpSrcEn        :1; // Bit 4
        uint8_t                      Done            :1; // Bit 5
        uint8_t                      Rsvd6To7        :2; //Bit 6 -7
        uint16_t                     BlockSize;
...
typedef struct _GDMA_CFG_REG_ {
    ...
    uint8_t                      SrcPer;
    uint8_t                      DestPer;
    uint16_t                     ChSusp      :1; //Bit 0
    uint16_t                     FifoEmpty   :1; //Bit 1
    uint16_t                     HsSelDst    :1; //Bit 2
    uint16_t                     HsSelSrc    :1; //Bit 3
    uint16_t                     LockCh      :1; //Bit 4
    uint16_t                     LockB       :1; //Bit 5
    uint16_t                     DstHsPol    :1; //Bit 6
    uint16_t                     SrcHsPol    :1; //Bit 7
    uint16_t                     ReloadSrc   :1; //Bit 8
    uint16_t                     ReloadDst   :1; //Bit 9
    uint16_t                     FifoMode    :1; //Bit 10
    uint16_t                     DsUpdEn     :1; //Bit 11
    uint16_t                     SsUpdEn     :1; //Bit 12
    uint16_t                     Rsvd13To15  :3;
