...
// int  VectorIrqDisRtl8195A(uint32_t); weak
// int  VectorIrqUnRegisterRtl8195A(uint32_t); weak
// int  HalPinCtrlRtl8195A(uint32_t, uint32_t, uint32_t); weak
// int VectorIrqRegisterRtl8195A(void); weak
// int  VectorIrqEnRtl8195A(uint32_t); weak
// int  HalDelayUs(uint32_t); weak
//-------------------------------------------------------------------------
// Data declarations
//-------------------------------------------------------------------------
...
HAL_Status SdioHostIsTimeout(uint32_t StartCount, uint32_t TimeoutCnt) {
	uint32_t t1, t2;
	HAL_Status result;
...
void SdioHostSendCmd(PSDIO_HOST_CMD Cmd) {
	...
	HAL_SDIOH_REG16(REG_SDIO_HOST_CMD) = *(uint16_t *) &Cmd->CmdFmt; // & 0x3FFB;
}
...
//-----
HAL_Status SdioHostSdClkCtrl(void *Data, int En, uint8_t Divisor) { // SD_CLK_DIVISOR
	PHAL_SDIO_HOST_ADAPTER psha = (PHAL_SDIO_HOST_ADAPTER)Data;
...
HAL_Status SdioHostErrIntRecovery(void *Data) {
	...
	uint16_t ierr = HAL_SDIOH_REG16(REG_SDIO_HOST_ERROR_INT_STATUS); // v40058032;
...
void SdioHostIsrHandle(void *Data) {
	PHAL_SDIO_HOST_ADAPTER psha = (PHAL_SDIO_HOST_ADAPTER)Data;
	uint16_t status = HAL_SDIOH_REG16(REG_SDIO_HOST_NORMAL_INT_STATUS);// v40058030;
	...
	if (status) {
		...
		{
			uint16_t val = HAL_SDIOH_REG16(REG_SDIO_HOST_NORMAL_INT_STATUS_EN);
		...
		if (status & NOR_INT_STAT_ERR_INT) // 0x8000 )
		{
			...
			uint16_t err = HAL_SDIOH_REG16(REG_SDIO_HOST_ERROR_INT_STATUS);
...
HAL_Status SdioHostChkCmdComplete(void *Data, uint32_t Timeout) {
{
	...
	if (psha) {
		if (Timeout == SDIO_HOST_WAIT_FOREVER) {
			...
			StartCount = HalTimerOp.HalTimerReadCount(1); //	v6 = (*((int (**)(uint32_t)) &HalTimerOp + 2))(1);
		}
		...
		uint8_t *StatusBuf)
{
	...
	*(uint32_t *) &Cmd.CmdFmt = Data;
	...
	v7 = Mode;
	v8 = Fn2Sel;
	if (!v6 || ((uint8_t) Fn1Sel | (uint8_t) v6) & 3) {
		result = 3;
	} else {
		...
		v9 = *(uint8_t *) v6;
		*(uint16_t *) (v6 + 2) = 64;
		*(uint8_t *) v6 = ((v9 | 3) & 0xFB | 4 * ((Fn1Sel | v6) & 1)) & 0xEF
				| 16 * ((Fn1Sel | v6) & 1) | 0x20;
		*(uint32_t *) (v6 + 4) = Fn1Sel;
		result = SdioHostChkCmdInhibitCMD();//v6);
		if (!result) {
			result = SdioHostChkDataLineActive();
			if (result == HAL_OK) {
				Cmd.CmdFmt = (SDIO_HOST_CMD_FMT) ((*(uint8_t *) &Cmd.CmdFmt & 0xF4
						| 0x3A) & 0x3F);
				v11 = *((uint8_t *) &Cmd.CmdFmt + 1);
				v5[128] = 0;
				*((uint8_t *) &Cmd.CmdFmt + 1) = v11 & 0xC0 | 6;
				v5[129] = 0;
				Cmd.Arg = v8 | 0xFFFFF0 | (v7 << 31);
				SdioHostSendCmd(&Cmd);
				result = SdioHostChkCmdComplete(v5);//, v12);
				if (!result) {
					SdioHostGetResponse(v5, *(uint8_t *) &Cmd.CmdFmt & 3);
...
//----- (000009CC) --------------------------------------------------------
HAL_Status HalSdioHostReadBlocksDmaRtl8195a(IN void *Data, IN uint64_t ReadAddr,
IN uint32_t BlockCnt) {
	...
	*(uint32_t *) &Cmd.CmdFmt = result;
	...
	if (BlockCnt >= 0x10000)
		return HAL_ERR_PARA;
	v6 = *(uint32_t *) (result + 16);
	if (v6 << 30 || *(uint32_t *) (*(uint32_t *) (result + 16) + 4) << 30)
		return 3;
	...
	while (1) {
		while (1) {
			...
			if (result)
				return result;
			Cmd.CmdFmt =
					(SDIO_HOST_CMD_FMT) ((*(uint8_t *) &Cmd.CmdFmt & 0xF4 | 0x3A)
							& 0x3F);
			v12 = *((uint8_t *) &Cmd.CmdFmt + 1);
			*(uint8_t *) (v5 + 128) = 0;
			*(uint8_t *) (v5 + 129) = 0;
			*((uint8_t *) &Cmd.CmdFmt + 1) = v12 & 0xC0 | 0x11;
			Cmd.Arg = v4;
			SdioHostSendCmd(&Cmd);
			result = SdioHostChkCmdComplete((void *) v5);//, v13);
			if (result)
				goto LABEL_21;
			SdioHostGetResponse((void *) v5, *(uint8_t *) &Cmd.CmdFmt & 3);
		...
		if (result)
			return result;
		Cmd.CmdFmt = (SDIO_HOST_CMD_FMT) ((*(uint8_t *) &Cmd.CmdFmt & 0xF4 | 0x3A)
				& 0x3F);
		v7 = *((uint8_t *) &Cmd.CmdFmt + 1);
		*(uint8_t *) (v5 + 128) = 0;
		*(uint8_t *) (v5 + 129) = 0;
		*(uint8_t *) (v5 + 130) = 0;
		*((uint8_t *) &Cmd.CmdFmt + 1) = v7 & 0xC0 | 0x12;
		Cmd.Arg = v4;
		SdioHostSendCmd(&Cmd);
...
//v5 = result; //pSdioHostAdapter
	...
	if (((uint32_t) AdmaDescTbl & 3) || (AdmaDescTbl.Addr1 & 3))
		return HAL_ERR_PARA;
	if (*(uint8_t *) (Data + 132))
	...
	while (1) {
		...
		while (1) {
			HAL_SDIO_HOST_WRITE32(REG_SDIO_HOST_ADMA_SYS_ADDR,
					(uint32_t)AdmaDescTbl); //40058058 = v6;
			...
			Cmd.CmdFmt =
					(SDIO_HOST_CMD_FMT) ((*(uint8_t *) &Cmd.CmdFmt & 0xF4 | 0x3A)
							& 0x3F);
			v10 = *((uint8_t *) &Cmd.CmdFmt + 1);
			*(uint8_t *) (pSdioHostAdapter + 128) = 0;
			*(uint8_t *) (pSdioHostAdapter + 129) = 0;
			*((uint8_t *) &Cmd.CmdFmt + 1) = v10 & 0xC0 | 0x18;
			Cmd.Arg = sec_count;
			SdioHostSendCmd(&Cmd);
			...
			SdioHostGetResponse((void *) pSdioHostAdapter,
					*(uint8_t *) &Cmd.CmdFmt & 3);
			if (*(uint32_t *) (pSdioHostAdapter + 20) & 0x4000000) {
				LABEL_14:
		...
		*(uint8_t *) (pSdioHostAdapter + 128) = 0;
		*(uint8_t *) (pSdioHostAdapter + 129) = 0;
		*(uint8_t *) (pSdioHostAdapter + 130) = 0;
		*((uint8_t *) &Cmd.CmdFmt + 1) = v7 & 0xC0 | 0x19;
		Cmd.Arg = sec_count;
		SdioHostSendCmd(&Cmd);
		result = SdioHostChkCmdComplete((void *) v5);//, v8);
		if (!result) {
...
HAL_Status HalSdioHostGetCardStatusRtl8195a(IN void *Data) {
	...
	*(uint32_t *) &Cmd.CmdFmt = Data;
	...
	if (result)
		return result;
	Cmd.CmdFmt =
			(SDIO_HOST_CMD_FMT) ((*(uint8_t *) &Cmd.CmdFmt & 0xF4 | 0x1A) & 0x1F);
	v5 = *((uint8_t *) &Cmd.CmdFmt + 1);
	*((uint8_t *) v3 + 128) = 0;
	*((uint8_t *) &Cmd.CmdFmt + 1) = v5 & 0xC0 | 0xD;
	Cmd.Arg = *((uint16_t *) v3 + 67) << 16;
	SdioHostSendCmd(&Cmd);
	v7 = SdioHostChkCmdComplete(v3);//, v6);
	if (v7)
		return v7;
	SdioHostGetResponse(v3, *(uint8_t *) &Cmd.CmdFmt & 3);
	if (*((uint8_t *) v3 + 24) == 13) {
		v8 = *((uint32_t *) v3 + 5);
		*((uint32_t *) v3 + 10) = v8;
		*((uint8_t *) v3 + 131) = (v8 >> 9) & 0xF;
		return v7;
	}
	DBG_SDIO_ERR("Command index error!\n");
	return 238;
...
HAL_Status HalSdioHostInitCardRtl8195a(IN void *Data) {
	...
	if (v9
			|| (LOBYTE(v52) = (((uint8_t) v52 & 0xF4 | 0x1A) & 0xDF | 32 * (v7 & 1))
		...
		SdioHostGetResponse(v3, (uint8_t) v52 & 3);
		if (v3[24] != 8) {
			if (ConfigDebugErr & 0x400) {
	...
	if (v13)
		goto LABEL_63;
	LOBYTE (v52) = ((uint8_t) v52 & 0xFC | 0x1A) & 0x1F;
	...
	if (v13)
		goto LABEL_63;
	v16 = SdioHostGetResponse(v3, (uint8_t) v52 & 3);
	if (v3[24] != 55) {
	...
	if (v13
			|| (LOBYTE(v52) = ((uint8_t) v52 & 0xFC | 2) & 7, v18 = BYTE1(v52) & 0xC0
	...
	v20 = SdioHostGetResponse(v3, (uint8_t) v52 & 3);
	v21 = 100;
	*((uint32_t *) v3 + 9) = *((uint32_t *) v3 + 5) & 0xFFFFFF;
	while (1) {
		v22 = __CFADD__(v21--, -1);
		if (!v22)
			goto LABEL_51;
		...
		if (v13)
			goto LABEL_63;
		v25 = SdioHostGetResponse(v3, (uint8_t) v52 & 3);
		if (v3[24] != 55) {
			if (ConfigDebugErr & 0x400) {
		...
		if (v13)
			goto LABEL_63;
		LOBYTE (v52) = ((uint8_t) v52 & 0xFC | 2) & 7;
		v27 = BYTE1(v52);
		v3[128] = 0;
	...
	LABEL_51: if (*((uint32_t *) v3 + 5) & 0x40000000) {
		v3[132] = 1;
	...
	if (v31
			|| (LOBYTE(v52) = ((uint8_t) v52 & 0xF4 | 9) & 0xF, v32 = BYTE1(v52)
	...
	v31 = SdioHostGetResponse(v3, (uint8_t) v52 & 3);
	LABEL_70: v34 = SdioHostChkCmdInhibitCMD();//v31);
	if (v34
			|| (LOBYTE(v52) = ((uint8_t) v52 & 0xF4 | 0x1A) & 0x1F, v35 = BYTE1(v52)
	...
	if (v42)
		goto LABEL_120;
	v47 = SdioHostGetResponse(v3, (uint8_t) v52 & 3);
	if (v3[24] != 55)
		goto LABEL_90;
	if (!(*((uint32_t *) v3 + 5) & 0x20)) {
		if (ConfigDebugErr & 0x400) {
	...
	if (v42
			|| (LOBYTE(v52) = ((uint8_t) v52 & 0xFC | 0x1A) & 0x1F, v49 = BYTE1(v52)
...
// 23D4: using guessed type int DiagPrintf(const char *, ...);
// 23F0: using guessed type int  HalDelayUs(uint32_t);
...
HAL_Status HalSdioHostGetSdStatusRtl8195a(IN void *Data) {
	...
	*(uint32_t *) &Cmd.CmdFmt = Data;
	...
	if (!v3 || v3 & 3) {
		...
		v5 = *(uint8_t *) v3;
		*(uint16_t *) (v3 + 2) = 64;
		v6 = (uint8_t) ((v5 | 3) & 0xEB) | 0x20;
		*(uint8_t *) v3 = v6;
		*(uint32_t *) (v3 + 4) = v4 + 48;
		result = SdioHostChkCmdInhibitCMD();//v6);
		if (!result) {
			Cmd.CmdFmt =
					(SDIO_HOST_CMD_FMT) ((*(uint8_t *) &Cmd.CmdFmt & 0xFC | 0x1A)
							& 0x1F);
			*((uint8_t *) &Cmd.CmdFmt + 1) = *((uint8_t *) &Cmd.CmdFmt + 1) & 0xC0 | 0x37;
			v8 = *((uint16_t *) v4 + 67);
			v4[128] = 0;
			Cmd.Arg = v8 << 16;
			SdioHostSendCmd(&Cmd);
			...
			if (!result) {
				SdioHostGetResponse(v4, *(uint8_t *) &Cmd.CmdFmt & 3);
				if (v4[24] != 55) {
					if (ConfigDebugErr & 0x400) {
				...
				result = SdioHostChkCmdInhibitCMD();//*((uint32_t *) v4 + 5) << 26);
				if (!result) {
					result = SdioHostChkDataLineActive(0);
					if (!result) {
						Cmd.CmdFmt = (SDIO_HOST_CMD_FMT) ((*(uint8_t *) &Cmd.CmdFmt
								& 0xFC | 0x3A) & 0x3F);
						v12 = *((uint8_t *) &Cmd.CmdFmt + 1);
						v4[128] = 0;
						v4[129] = 0;
						Cmd.Arg = 0;
						*((uint8_t *) &Cmd.CmdFmt + 1) = v12 & 0xC0 | 0xD;
						SdioHostSendCmd(&Cmd);
...
HAL_Status HalSdioHostChangeSdClockRtl8195a(IN void *Data, IN uint8_t Frequency) {
	PHAL_SDIO_HOST_ADAPTER pSdioHostAdapter = (PHAL_SDIO_HOST_ADAPTER) Data; //	v2 = Data; uint8_t * v2; // r5@1
	PHAL_SDIO_HOST_ADAPTER psha = (PHAL_SDIO_HOST_ADAPTER)Data;
	...
	*(uint32_t *) &StatusData[4] = 0;
	...
	//	v5 = (uint8_t) ((*(uint8_t *) v4 | 3) & 0xEB) | 0x20;
	//	*(uint8_t *) v4 = v5;
	pAdmaDescTbl->Attrib1.Int = 0;
	pAdmaDescTbl->Attrib1.Act1 = 1;
	pAdmaDescTbl->Len1 = 8; // *(uint16_t *) (v4 + 2) = 8;
	pAdmaDescTbl->Addr2 = &StatusData; // *(uint32_t *) (v4 + 4) = StatusData;
	...
	sdhcmd.Arg = pSdioHostAdapter->Csd[?] << 16; //	v24 = *((uint16_t *) v2 + 67) << 16;
	...
	if (v6 || (v6 = SdioHostChkDataLineActive(0)) != 0
		...
		v3 = 0;
		*((uint16_t *) v2 + 68) = StatusData[0] & 0xF;
	}
	if (*((uint16_t *) v2 + 68)) {
...
IN uint64_t EndAddr) {
	...
	if (!(uint32_t) EndAddr)
		return 3;
	if (*(uint8_t *) (EndAddr + 132)) {
		v4 = a2 >> 9;
		v3 = EndAddra >> 9;
	}
	result = SdioHostChkCmdInhibitCMD();//EndAddr);
	if (!result) {
		v16.CmdFmt = (SDIO_HOST_CMD_FMT) ((*(uint8_t *) &v16.CmdFmt & 0xFC | 0x1A)
				& 0x1F);
		v7 = *((uint8_t *) &v16.CmdFmt + 1);
		*(uint8_t *) (v5 + 128) = 0;
		*((uint8_t *) &v16.CmdFmt + 1) = v7 & 0xC0 | 0x20;
		v16.Arg = v4;
		SdioHostSendCmd(&v16);
		result = SdioHostChkCmdComplete((void *) v5);
		if (!result) {
			v9 = SdioHostGetResponse((void *) v5, *(uint8_t *) &v16.CmdFmt & 3);
			if (*(uint8_t *) (v5 + 24) != 32)
				goto LABEL_20;
			...
			if (result)
				return result;
			v16.CmdFmt =
					(SDIO_HOST_CMD_FMT) ((*(uint8_t *) &v16.CmdFmt & 0xFC | 0x1A)
							& 0x1F);
			v10 = *((uint8_t *) &v16.CmdFmt + 1);
			*(uint8_t *) (v5 + 128) = 0;
			*((uint8_t *) &v16.CmdFmt + 1) = v10 & 0xC0 | 0x21;
			v16.Arg = v3;
			SdioHostSendCmd(&v16);
			result = SdioHostChkCmdComplete((void *) v5);
			if (result)
				return result;
			v12 = SdioHostGetResponse((void *) v5, *(uint8_t *) &v16.CmdFmt & 3);
			if (*(uint8_t *) (v5 + 24) != 33) {
				LABEL_20:
				DBG_SDIO_ERR("Command index error!\n");
				result = 238;
				...
				if (!result) {
					result = SdioHostChkCmdInhibitDAT(0);
					if (!result) {
						v16.CmdFmt = (SDIO_HOST_CMD_FMT) ((*(uint8_t *) &v16.CmdFmt
								| 0x1B) & 0x1F);
						v13 = *((uint8_t *) &v16.CmdFmt + 1);
						*(uint8_t *) (v5 + 128) = 0;
						*(uint8_t *) (v5 + 129) = 0;
						v16.Arg = 0;
						*((uint8_t *) &v16.CmdFmt + 1) = v13 & 0xC0 | 0x26;
						SdioHostSendCmd(&v16);
						result = SdioHostChkCmdComplete((void *) v5);
						if (!result)
...
HAL_Status HalSdioHostGetWriteProtectRtl8195a(IN void *Data) {
	...
	if (*((uint8_t *) v3 + 131) == 3) {
		...
		if (!v6) {
			*((uint32_t *) v3 + 11) = ((unsigned int) *((uint8_t *) v3 + 126) >> 4) & 1;
	...
	if (*((uint8_t *) v3 + 131) == 4 || *((uint8_t *) v3 + 131) == 5) {
...
//----- (00001B48) --------------------------------------------------------
HAL_Status HalSdioHostSetWriteProtectRtl8195a(IN void *Data, IN uint8_t Setting) {
	int v2; // r3@1
	uint8_t *v3; // r4@1
	int v4; // r2@2
	uint8_t v5; // r2@5
	...
	if (!v2)
		return 3;
	v4 = *((uint32_t *) Data + 4) & 3;
	if (v2 & 3)
		return 3;
	do {
		tmp[v4] = *((uint8_t *) Data + v4 + 112);
		++v4;
	} while (v4 != 15);
	if (Setting == 1)
		v5 = tmp[14] | 0x10;
