...
typedef struct _RUART_SPEED_SETTING_ {
    ...
    uint8_t Ovsr_adj_max_bits;  // 9: No parity, 10: with Parity
    uint8_t Ovsr_adj_bits;
    uint16_t *Ovsr_adj_map;
    uint32_t max_err;        // 10 ~ 100: 30
    uint32_t Ovsr_min;       // 10 ~ 20: 1000
    uint32_t Ovsr_max;       // 10 ~ 20: 2000
    uint32_t divisor_resolution; // 1 ~ 20: 10
    uint32_t jitter_lim;     // 50 ~ 100: 100
    uint32_t sclk;           // 83.33333 MHz
#endif    
...
FindElementIndex_v02(
        uint32_t Element,  ///< RUART Baudrate
        uint32_t* Array,    ///< Pre-defined Baudrate Array
        uint32_t ElementNo
);
