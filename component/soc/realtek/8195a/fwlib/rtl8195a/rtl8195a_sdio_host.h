...
{
	uint16_t RespType:2;		//bit:0..1
	uint16_t Rsvd0:1;		//bit:2
	uint16_t CmdCrcChkEn:1;	//bit:3
	uint16_t CmdIdxChkEn:1;	//bit:4
	uint16_t DataPresent:1;	//bit:5
	uint16_t CmdType:2;		//bit:6..7
	uint16_t CmdIdx:6;		//bit:8..13
	uint16_t Rsvd1:2;		//bit:14..15
...
HalSdioHostChangeSdClockRtl8195a(
	IN void *Data,
	IN uint8_t Frequency
);

HAL_Status
HalSdioHostEraseRtl8195a(
	IN void *Data,
	IN uint64_t StartAddr,
	IN uint64_t EndAddr
);
...
typedef struct _COMMAND_FORMAT_
{
	...
	uint16_t Rsvd0:1;
	uint16_t CMD_CRC_Chk:1;
	uint16_t CMD_Idx_Chk:1;
...
{
	uint8_t Data1[512];  /* 1st buffer */
	uint8_t Data2[512];  /* 2nd buffer */
