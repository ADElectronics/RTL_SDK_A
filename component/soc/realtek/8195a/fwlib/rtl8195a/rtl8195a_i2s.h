...
typedef struct _I2S_CTL_REG_ {
    ...
    uint32_t                    I2s_En         :1; // Bit 0
    uint32_t                    Rsvd1to4       :4; // Bit 1-4 is TrxAct, ChNum
    uint32_t                    EdgeSw         :1; // Bit 5 Edge switch
    uint32_t                    WordLength     :1; // Bit 6
    uint32_t                    LoopBack       :1; // Bit 7
    uint32_t                    Rsvd8to9       :2; // Bit 8-9 is Format
    uint32_t                    DacLrSwap      :1; // Bit 10
    uint32_t                    SckInv         :1; // Bit 11
    uint32_t                    EndianSwap     :1; // Bit 12
    uint32_t                    Rsvd13to14     :2; // Bit 11-14
    uint32_t                    DebugSwitch    :2; // Bit 15-16
    uint32_t                    Rsvd17to28    :12; // Bit 17-28
    uint32_t                    SlaveMode      :1; // Bit 29
    uint32_t                    SR44p1KHz      :1; // Bit 30
    uint32_t                    SwRstn         :1; // Bit 31
