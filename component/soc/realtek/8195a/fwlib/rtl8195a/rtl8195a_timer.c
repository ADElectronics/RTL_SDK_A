...
HAL_RAM_BSS_SECTION uint32_t gTimerRecord;
#endif

#if defined(CONFIG_CHIP_C_CUT) || defined(CONFIG_CHIP_E_CUT)
extern uint32_t Timer2To7HandlerData[MAX_TIMER_VECTOR_TABLE_NUM];
#else
uint32_t Timer2To7HandlerData[MAX_TIMER_VECTOR_TABLE_NUM];
#endif
...
{
    uint32_t TimerIrqStatus = 0, CheckIndex;
    IRQ_FUN pHandler;
...
)
{
    ...
    if (pHalTimerAdap->TimerId > 7) {
        ...
        TimerIrqHandle.Data = (uint32_t)pHalTimerAdap;
...
)
{
    ...
    ControlReg = ((uint32_t)pHalTimerAdap->TimerMode<<1)|((uint32_t)pHalTimerAdap->IrqDis<<2);
    /*
        set TimerControlReg
...
{
    uint32_t TimerCountOld;
    uint32_t TimerCountNew;
    uint32_t TimerRDCnt;
