...
extern void SleepPG(uint8_t  Option, uint32_t SDuration);
extern void PSHalInitPlatformLogUart(void);
extern void HalReInitPlatformTimer(void);
extern void DeepStandby(uint8_t  Option, uint32_t SDuration, uint8_t  GpioOption);
extern void QueryRegPwrState(uint8_t  FuncIdx, uint8_t* RegState, uint8_t* HwState);
#endif
...
uint8_t SDIO_Send_C2H_PktMsg(
	IN PHAL_SDIO_ADAPTER pSDIODev, 
	IN uint8_t *C2HMsg, 
	IN uint16_t MsgLen
);

uint8_t SDIO_Process_RPWM(
	IN PHAL_SDIO_ADAPTER pSDIODev
);

uint8_t SDIO_Process_RPWM2(
	IN PHAL_SDIO_ADAPTER pSDIODev
);
...
void SDIO_Register_Tx_Callback(
	IN PHAL_SDIO_ADAPTER pSDIODev,
	IN int8_t (*CallbackFun)(void *pAdapter, uint8_t *pPkt, uint16_t Offset, uint16_t PktSize),
	IN void *pAdapter	
);

int8_t SDIO_Rx_Callback(
	IN PHAL_SDIO_ADAPTER pSDIODev,
	IN void *pData,
	IN uint16_t Offset,
	IN uint16_t Length,
...
// TODO: initial TX BD
	...
	pSDIODev->pTXBDAddrAligned = (PSDIO_TX_BD)(((((uint32_t)pSDIODev->pTXBDAddr - 1) >> 2) + 1) << 2);	// Make it 4-bytes aligned
...
//allocate wlan skb here
	...
	if(pTxBdHdl->skb)
		pTxBdHdl->pTXBD->Address = (uint32_t)pTxBdHdl->skb->tail;
	else
		DBG_SDIO_ERR("SDIO_Device_Init: rltk_wlan_alloc_skb (%d) failed!\n", SDIO_TX_BD_BUF_USIZE*SDIO_TX_BUF_SZ_UNIT);
#else
		pTxBdHdl->pTXBD->Address = (uint32_t)(&inic_TX_Buf[i][0]);
#endif
#else
        // Allocate buffer for each TX BD
        pTxBdHdl->pTXBD->Address = (uint32_t)RtlMalloc(SDIO_TX_BD_BUF_USIZE*SDIO_TX_BUF_SZ_UNIT);
#if SDIO_DEBUG
        pSDIODev->MemAllocCnt++;
#endif
#endif
        if (NULL == (uint32_t*)(pTxBdHdl->pTXBD->Address)) {
...
#endif
			pTxBdHdl->pTXBD->Address =(uint32_t)NULL;
#else
                    RtlMfree((uint8_t *)pTxBdHdl->pTXBD->Address, (SDIO_TX_BD_BUF_USIZE*SDIO_TX_BUF_SZ_UNIT));
#endif
                }
            }
		    goto SDIO_INIT_ERR;            
	...
	pSDIODev->pRXBDAddrAligned = (PSDIO_RX_BD)(((((uint32_t)pSDIODev->pRXBDAddr - 1) >> 3) + 1) << 3);	// Make it 8-bytes aligned
...
//End
	...
	if (pSDIODev->pRxPktHandler) {
		RtlMfree((uint8_t*)pSDIODev->pRxPktHandler, sizeof(SDIO_RX_PACKET)*SDIO_RX_PKT_NUM);
		pSDIODev->pRxPktHandler = NULL;
	}

	if (pSDIODev->pRXBDHdl) {
		RtlMfree((uint8_t *)pSDIODev->pRXBDHdl, SDIO_RX_BD_NUM * sizeof(SDIO_RX_BD_HANDLE));
		pSDIODev->pRXBDHdl = NULL;
	}

	if (pSDIODev->pRXBDAddr) {
		RtlMfree((uint8_t *)pSDIODev->pRXBDAddr, (SDIO_RX_BD_NUM * sizeof(SDIO_RX_BD))+7);
		pSDIODev->pRXBDAddr = NULL;
	}
#if (CONFIG_INIC_EN == 0)
    if (pSDIODev->pTxPktHandler) {
        RtlMfree((uint8_t *)pSDIODev->pTxPktHandler, (sizeof(SDIO_TX_PACKET)*SDIO_TX_PKT_NUM));
    ...
    if (pSDIODev->pTXBDHdl) {
        RtlMfree((uint8_t *)pSDIODev->pTXBDHdl, (SDIO_TX_BD_NUM * sizeof(SDIO_TX_BD_HANDLE)));
...
)
{
	...
	while (1) {
		...
		if (SDIO_IsEventPending(pSDIODev, SDIO_EVENT_TX_STOPPED) && 
            SDIO_IsEventPending(pSDIODev, (uint32_t)SDIO_EVENT_RX_STOPPED)) {
...
)
{
	...
	SdioIrqHandle.Data = (uint32_t) pSDIODev;
...
#if !TASK_SCHEDULER_DISABLED
    SDIO_SetEvent(pSDIODev, (uint32_t)SDIO_EVENT_RX_STOPPED);
...
)
{
	...
	volatile uint16_t TxBDWPtr=0;
	uint32_t processed_pkt_cnt=0;
	uint8_t isForceBreak=0;
	int8_t ret=FAIL;
    uint32_t mem_alloc_failed=0;
    uint32_t reg;
...
#if CONFIG_INIC_EN
#if CONFIG_INIC_SKB_TX
		ret = pSDIODev->Tx_Callback(pSDIODev->pTxCb_Adapter, (uint8_t*)pTxBdHdl->skb, pTxDesc->offset, pTxDesc->txpktsize);  // includes TX Desc
#else
		ret = pSDIODev->Tx_Callback(pSDIODev->pTxCb_Adapter, (uint8_t*)pTxBdHdl->pTXBD->Address, pTxDesc->offset, pTxDesc->txpktsize);  // includes TX Desc
#endif
#else
                ret = pSDIODev->Tx_Callback(pSDIODev->pTxCb_Adapter, (uint8_t*)pTxBdHdl->pTXBD->Address, pTxDesc->offset, pTxDesc->txpktsize);  // includes TX Desc
#endif
                ...
                ret = pSDIODev->Tx_Callback(pSDIODev->pTxCb_Adapter,                            // doesn't include TX Desc
                                            (uint8_t*)(pTxBdHdl->pTXBD->Address+pTxDesc->offset), 
                                            pTxDesc->txpktsize);
#endif
            }
            else {
...
{
    if (pPkt->isDyna) {
        RtlMfree((uint8_t *)pPkt, sizeof(SDIO_RX_PACKET));
    }
    else {
    	RtlDownMutex(&pSDIODev->RxMutex);
    	RtlListInsertTail(&pPkt->list, &pSDIODev->FreeRxPktList);
...
)
{
	...
	SDIO_RX_BD *pRXBD;
	uint32_t PktSize;
	uint32_t FreeCnt=0;		// for debugging
...
#else
			inic_sdio_free_data((uint8_t *) (pRxBdHdl->pPkt->pData));
#endif
			pRxBdHdl->pPkt->pData = NULL;
#else
				    RtlMfree((uint8_t *) (pRxBdHdl->pPkt->pData), (pRxBdHdl->pPkt->Offset+PktSize));	// free packet buffer
#if SDIO_DEBUG
...
)
{
	...
	TmpC2HMsg = ((TmpC2HMsg ^ (uint32_t)BIT(31)) & (uint32_t)BIT(31)) | *C2HMsg;
...
void SDIO_Pre_Sleep_Callback(uint32_t expected_idle_time){
	/* Indicate the Host system that the TX/RX is not ready */
	HAL_SDIO_WRITE8(REG_SPDIO_CPU_IND, \
		HAL_SDIO_READ8(REG_SPDIO_CPU_IND)&~BIT_SYSTEM_TRX_RDY_IND);
}
void SDIO_Post_Sleep_Callback(uint32_t expected_idle_time){
	/* Indicate the Host system that the TX/RX is ready */
	HAL_SDIO_WRITE8(REG_SPDIO_CPU_IND, \
		HAL_SDIO_READ8(REG_SPDIO_CPU_IND)|BIT_SYSTEM_TRX_RDY_IND);
}
...
)
{
	...
	SDIO_RX_BD *pRXBD;
	uint32_t Offset=0;
	uint16_t RxBdWrite=0;	// to count how much RX_BD used in a Transaction
	uint16_t RxBdRdPtr=0;	// RX_BD read pointer
	uint32_t pkt_size;
	uint8_t isForceBreak=0;
    uint8_t isListEmpty;
#if SDIO_RX_PKT_SIZE_OVER_16K
	uint8_t needed_rxbd_num;
#endif
...
//		DBG_SDIO_INFO("SDIO_Return_Rx_Data: Queue is empty\n");
	...
	do {
		...
		pRxBdHdl->pPkt = pPkt;
		pRXBD->FS = 1;
		pRXBD->PhyAddr = (uint32_t)((uint8_t *)pRxDesc);
		...
		do {
			...
			pRxBdHdl->pPkt = pPkt;
			pRXBD->FS = 0;
			pRXBD->PhyAddr = (uint32_t)(((uint8_t *)pPkt->pData)+pPkt->Offset);
#if SDIO_RX_PKT_SIZE_OVER_16K
...
)
{
	...
	case MSG_SDIO_RX_PKT:
        ...
        if (SUCCESS != ret) {
            ...
            RtlMfree((uint8_t *) pMblk->pBuf, (pMblk->Reserved + pMblk->DateLen));   // pMblk->Reserved = Offset
#if SDIO_DEBUG
            RtlDownMutex(&pSDIODev->StatisticMutex);
            pSDIODev->MemAllocCnt--;
            RtlUpMutex(&pSDIODev->StatisticMutex);
    ...
    for (;;)
    {
        ...
        while (!RtlIsListEmpty(&pSDIODev->MP_RxPktList)) {
            ...
            if (_SUCCESS != RtlMailboxSendToBack(MBOX_ID_SDIO, &Mblk_w, 2000, 0)) {
                ...
                RtlMfree((uint8_t *)pRxPkt, sizeof(SDIO_MP_RX_PACKET));
            }
        }
...
)
{
    ...
    uint32_t oldest_byte_cnt;
    uint32_t tx_avg_tp=0;  // in Kbps
    uint32_t rx_avg_tp=0;  // in Kbps
...
)
{
	...
	case SDIO_MP_LOOPBACK:
		...
		arg1 = Strtoul((const uint8_t*)(argv[1]), (uint8_t **)NULL, 10);
		if (arg1) {
	...
	case SDIO_MP_READ_REG8:
		arg1 = Strtoul((const uint8_t*)(argv[1]), (uint8_t **)NULL, 16);
	...
	case SDIO_MP_WRITE_REG8:
		arg1 = Strtoul((const uint8_t*)(argv[1]), (uint8_t **)NULL, 16);
		arg2 = Strtoul((const uint8_t*)(argv[2]), (uint8_t **)NULL, 16);
    ...
    case SDIO_MP_CRX_DA:
		...
		arg2 = Strtoul((const uint8_t*)(argv[2]), (uint8_t **)NULL, 10);
        ...
        if (SDIO_CRX_STATIC_BUF == pSDIODev->MP_ContinueRxMode) {
            if (NULL == pSDIODev->pMP_CRxBuf) {
                ...
                DiagPrintf("SDIO RX Test: pBuf @ 0x%x\n", (uint32_t)pSDIODev->pMP_CRxBuf);
                if (((uint32_t)(pSDIODev->pMP_CRxBuf) & 0x03) != 0) {
