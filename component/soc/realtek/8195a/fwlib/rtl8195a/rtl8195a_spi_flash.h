...
void SpicInitRtl8195A(uint8_t InitBaudRate, uint8_t SpicBitMode);			// spi-flash controller initialization
void SpicRxCmdRtl8195A(uint8_t cmd); 								// recieve command
void SpicTxCmdRtl8195A(uint8_t cmd, SPIC_INIT_PARA SpicInitPara);	// transfer command
uint8_t SpicGetFlashStatusRtl8195A(SPIC_INIT_PARA SpicInitPara);		// RDSR, read spi-flash status register
void SpicSetFlashStatusRtl8195A(uint32_t data, SPIC_INIT_PARA SpicInitPara);		// WRSR, write spi-flash status register
...
uint32_t SpicCmpDataForCalibrationRtl8195A(void);			// compare read_data and golden_data
void SpicLoadInitParaFromClockRtl8195A(uint8_t CpuClkMode, uint8_t BaudRate, PSPIC_INIT_PARA pSpicInitPara); 

#if 1//CONFIG_CHIP_E_CUT

void 
SpicTxCmdWithDataRtl8195A_V04(
    IN  SPIC_INIT_PARA *pSpicInitPara,
    IN  uint8_t cmd, 
    IN  uint8_t DataPhaseLen, 
    IN  uint8_t* pData
);

void 
SpicTxFlashInstRtl8195A_V04(
    IN  SPIC_INIT_PARA *pSpicInitPara,
    IN  uint8_t cmd, 
    IN  uint8_t DataPhaseLen, 
    IN  uint8_t* pData
...
void 
SpicDeepPowerDownFlashRtl8195A_V04(
    IN  SPIC_INIT_PARA *pSpicInitPara    
);

void 
SpicBlockEraseFlashRtl8195A_V04(
    IN  SPIC_INIT_PARA *pSpicInitPara,
    IN uint32_t Address
);

void 
SpicSectorEraseFlashRtl8195A_V04(
    IN  SPIC_INIT_PARA *pSpicInitPara,
    IN uint32_t Address
);

void 
SpicWriteProtectFlashRtl8195A_V04(
    IN  SPIC_INIT_PARA *pSpicInitPara,
    IN uint32_t Protect
);

BOOLEAN
SpicFlashInitRtl8195A_V04(
    IN  SPIC_INIT_PARA *pSpicInitPara,
    IN  uint8_t SpicBitMode
);

uint32_t 
SpicCalibrationRtl8195A_V04(
    IN  SPIC_INIT_PARA *pSpicInitPara,
    IN  uint8_t SpicBitMode,
    IN  uint32_t DefRdDummyCycle
);

void 
SpicConfigAutoModeRtl8195A_V04(
    IN  uint8_t SpicBitMode
);

void 
SpicWaitWipDoneRefinedRtl8195A_V04(
    IN  SPIC_INIT_PARA *pSpicInitPara
);

void 
SpicRxCmdRefinedRtl8195A_V04(
    IN  SPIC_INIT_PARA *pSpicInitPara,
    IN  uint8_t cmd
);

uint8_t 
SpicGetFlashStatusRefinedRtl8195A_V04(
    IN  SPIC_INIT_PARA *pSpicInitPara
);

void 
SpicInitRefinedRtl8195A_V04(
    IN  SPIC_INIT_PARA *pSpicInitPara,
...
void 
SpicSetFlashStatusRefinedRtl8195A_V04(
    IN  SPIC_INIT_PARA *pSpicInitPara,
    IN  uint32_t data
);

uint32_t
SpicWaitWipRtl8195A_V04(
    IN  SPIC_INIT_PARA *pSpicInitPara    
);

uint32_t
SpicOneBitCalibrationRtl8195A_V04(
    IN  SPIC_INIT_PARA *pSpicInitPara,
    IN uint8_t SysCpuClk 
);

void 
SpicLoadInitParaFromClockRtl8195A_V04(
...
uint8_t
SpicGetFlashFlagRtl8195A_V04(IN SPIC_INIT_PARA * pSpicInitPara);

void 
SpicWaitOperationDoneRtl8195A_V04(IN SPIC_INIT_PARA * pSpicInitPara);

void
SpicUserProgramRtl8195A_V04(IN uint8_t * data, IN SPIC_INIT_PARA SpicInitPara, IN uint32_t addr, IN uint32_t * LengthInfo);

void
SpicUserReadRtl8195A_V04(IN uint32_t Length, IN uint32_t addr, IN uint8_t * data);

void
SpicUserReadFourByteRtl8195A_V04(IN uint32_t Length, IN uint32_t addr, IN uint32_t * data, IN uint8_t BitMode);

void
SpicSetExtendAddrRtl8195A_V04(IN uint32_t data, IN SPIC_INIT_PARA * pSpicInitPara);

uint8_t
SpicGetExtendAddrRtl8195A_V04(IN SPIC_INIT_PARA * pSpicInitPara);

void
SpicReadIDRtl8195A_V04(void);

void
SpicDieEraseFlashRtl8195A_V04(IN SPIC_INIT_PARA * pSpicInitPara, IN uint32_t Address);

uint8_t
SpicGetConfigRegRtl8195A_V04(IN SPIC_INIT_PARA * pSpicInitPara);
