...
HAL_Status SdioHostSdClkCtrl(void *Data, int En, uint8_t Divisor) { // SD_CLK_DIVISOR
	PHAL_SDIO_HOST_ADAPTER psha = Data; // uint8_t *v3; // r3@1 	v3 = Data;
...
void SdioHostSendCmd(PSDIO_HOST_CMD Cmd) {
	//uint16_t reg_cmd = (*(uint8_t *) &Cmd->CmdFmt & 0x3B) | (*(uint8_t *) &Cmd->CmdFmt & 0xC0)
	//		| ((*((uint8_t *)&Cmd->CmdFmt + 1) & 0x3F) << 8);
	Cmd->CmdFmt.Rsvd0=0;
	Cmd->CmdFmt.Rsvd1=0;
...
void SdioHostIsrHandle(void *Data) {
	...
	uint32_t status = HAL_SDIOH_REG16(REG_SDIO_HOST_NORMAL_INT_STATUS);// v40058030;
...
// так в sdk
//----- (000009CC) --------------------------------------------------------
HAL_Status HalSdioHostReadBlocksDmaRtl8195a_sdk(IN void *Data, IN uint64_t ReadAddr,
		IN uint32_t BlockCnt) {
	PHAL_SDIO_HOST_ADAPTER psha = (PHAL_SDIO_HOST_ADAPTER)Data;
	SDIO_HOST_CMD Cmd;
	HAL_Status result;
