...
)
{
    ...
    uint8_t UartIndex = pHalRuartAdapter->UartIndex;
    uint32_t rx_trigger_lv;
...
{
    uint32_t baud_rate;
    uint32_t min_divisor=0;
    uint32_t min_err=0xffffffff;
    uint32_t uart_ovsr;
    uint32_t min_uart_ovsr = 0;  // ovsr with mini err
    uint32_t min_uart_ovsr_mod = 0;
    uint64_t uart_clock;
    uint32_t divisor_temp;
    uint32_t max_jitter_temp;
    uint32_t err_temp;
    uint32_t uart_ovsr_target;
    uint32_t uart_ovsrs_actual;
    uint32_t ovsr_adj;
    uint32_t adj_bits;
    uint32_t div_res;
    uint32_t uart_ovsrs_actual_mod;

    baud_rate = pBaudSetting->BaudRate;
    if (baud_rate >= 1000000) {
        baud_rate /= 100;
        uart_clock = (uint64_t)pBaudSetting->sclk;
    } else {
        baud_rate /= 2;
        uart_clock = (uint64_t)pBaudSetting->sclk * 50;   // UART clock is 1/2 CPU clock
    }
...
//                    err_temp = (uart_clock/divisor_temp)/((uart_ovsr/100)*100);
                    ...
                    if (err_temp < min_err) {
                        ...
                        uint32_t uart_ovsr_mod = uart_ovsr%100;
...
)
{
    ...
    if ((pHalGdmaAdapter->MuliBlockCunt == pHalGdmaAdapter->MaxMuliBlock)) {   
        ...
        UartIndex = pHalRuartAdapter->UartIndex;
        LineStatus = (uint8_t)HAL_RUART_READ32(UartIndex, RUART_LINE_STATUS_REG_OFF);
...
//    uint8_t UartIndex = pHalRuartAdapter->UartIndex;
    ...
    if (((pHalRuartAdapter->TxCount & 0x03)==0) &&
        (((uint32_t)(pHalRuartAdapter->pTxBuf) & 0x03)==0)) {
    ...
    pHalGdmaAdapter->ChSar = (uint32_t)(pHalRuartAdapter->pTxBuf);
...
)
{
    ...
    if (((uint32_t)(pHalRuartAdapter->pRxBuf) & 0x03)==0) {
    ...
    pHalGdmaAdapter->MuliBlockCunt = 0;
    pHalGdmaAdapter->MaxMuliBlock = 1;
    pHalGdmaAdapter->ChDar = (uint32_t)(pHalRuartAdapter->pRxBuf);
...
)
{
    ...
    BlkNum = 0;
    SrcAddr = (uint32_t)pTxData;
    for (i=0; i<UART_DMA_MBLK_NUM; i++) {
        pDmaBlkList->GdmaChLli[i].Sarx = SrcAddr;
        pDmaBlkList->GdmaChLli[i].Darx = (uint32_t) (pHalGdmaAdapter->ChDar);
...
)
{
    ...
    BlkNum = 0;
    DstAddr = (uint32_t)pRxData;
    for (i=0; i<UART_DMA_MBLK_NUM; i++) {
        pDmaBlkList->GdmaChLli[i].Sarx = (uint32_t) (pHalGdmaAdapter->ChSar);
...
//            uint8_t  IsrTypeMap =
            ...
            RecvdCnt = DMA_Dar - (uint32_t)(pHalRuartAdapter->pRxBuf);
...
//            uint8_t  IsrTypeMap =
            ...
            TxedCnt = DMA_Sar - (uint32_t)(pHalRuartAdapter->pTxBuf);
//            DBG_8195A("%s: got %d bytes\r\n", __FUNCTION__, RecvdCnt);
