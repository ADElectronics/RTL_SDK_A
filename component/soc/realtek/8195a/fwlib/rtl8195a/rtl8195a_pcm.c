...
)
{
    ...
    PHAL_PCM_ADAPTER pHalPcmAdapter = (PHAL_PCM_ADAPTER) Data;
    uint8_t  PcmIndex = pHalPcmAdapter->PcmIndex;
    uint8_t  PcmCh = pHalPcmAdapter->PcmCh;
    uint32_t RegCtl, RegChCNR03, RegTSR03, RegBSize03;
    uint32_t Isr03;
...
)
{
    ...
    uint32_t IsrMask, Addr, IsrCtrl;
    uint8_t  IsrTypeIndex = 0;
...
)
{
    PHAL_GDMA_ADAPTER pHalGdmaAdapter = (PHAL_GDMA_ADAPTER) Data;
    uint32_t IsrStatus;
    uint8_t  IsrTypeIndex = 0, IsrActBitMap = 0;
