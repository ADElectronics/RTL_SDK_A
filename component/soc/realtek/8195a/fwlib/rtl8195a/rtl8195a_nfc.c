...
extern void SpicSectorEraseFlashRtl8195A(uint32_t Address);
...
void N2AReadTag(IN void *pNFCAdapte, IN uint8_t N2ARPage);
uint32_t HalNFCRead32(IN  uint32_t Addr);
void HalNFCWrite32(IN  uint32_t Addr, IN uint32_t Data);
void N2AWriteTag(IN void *pNFCAdapte, IN uint8_t N2AWPage);
void N2AReadCatch(IN void *pNFCAdapte, IN uint8_t N2ACatchRPage);
void NFCReaderPresent(IN void *pNFCAdapte, IN uint8_t State);
void N2AMailboxState(IN void *pNFCAdapte, IN uint8_t State, IN uint8_t Seq);
void NFC25MClkReq(IN void *pNFCAdapte, IN uint8_t OP, IN uint8_t Seq);
...
uint32_t LpPeriIrqDataTable[32];
...
)
{
    PNFC_ADAPTER pNFCAdp = (PNFC_ADAPTER)pNFCAdapte;
    A2N_MAILBOX_Q MailData;
    uint32_t RegTemp = HAL_READ32(SYSTEM_CTRL_BASE, REG_SYS_SYSPLL_CTRL1);
    if (OP) {
...
)
{
    ...
    if (pNFCAdp->NFCIsr & (BIT0|BIT6)) {
        ...
        while (N2AMISC != 0){
            ...
            N2ARB2 = ((uint8_t)(N2ARData>>16));
            Seq = (uint8_t)(N2ARData>>8);
...
{
    uint32_t LpIrqStatus, CheckIndex, ExactIrqStatus, TableIndex;
...
{
    PNFC_ADAPTER pNFCAdp = (PNFC_ADAPTER)pNFCAdapte;
    uint32_t ISR = HalNFCRead32(0x68);
    pNFCAdp->NFCIsr = (((uint8_t)((ISR&0xbf00)>>8))&((uint8_t)ISR));
...
)
{
    ...
    uint32_t pgidx, dmemidx;
    for (pgidx = 0, dmemidx = 4; pgidx<TagLen; pgidx++) {
        NFCFWDMEM[dmemidx++] = (uint8_t)((pTagData[pgidx] & 0x000000FF) >>  0);
        NFCFWDMEM[dmemidx++] = (uint8_t)((pTagData[pgidx] & 0x0000FF00) >>  8);
        NFCFWDMEM[dmemidx++] = (uint8_t)((pTagData[pgidx] & 0x00FF0000) >> 16);
        NFCFWDMEM[dmemidx++] = (uint8_t)((pTagData[pgidx] & 0xFF000000) >> 24);
    }
}
...
)
{
    ...
    //Init INT
    {
        ...
        NfcHandle.Data       = (uint32_t) (pNFCAdp);
        ...
        LpPeriIrqFunTable[1]  = (IRQ_FUN)((uint32_t)NFCIrqHandle | 0x1);

        LpPeriIrqDataTable[1] = (uint32_t)(pNFCAdp);
        //level trigger
...
HalNFCFwFullMEM(
    IN  void *Data,
    IN  uint32_t  NFCFwLength
)
{
    uint32_t *NFCFW;
    NFCFW = (uint32_t*) Data;
    uint32_t Idx, NFCFwTemp, RTemp;
    uint8_t Temp = (NFCFwLength%4);
    uint32_t IdxTemp = ((NFCFwLength-Temp)/4);
    uint8_t MEMTemp = 0;
//    uint8_t MEMCnt = 0;
