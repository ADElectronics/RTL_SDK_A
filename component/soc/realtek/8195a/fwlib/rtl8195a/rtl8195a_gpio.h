...
typedef struct {
	...
	uint32_t GPIO_Pin;    // Pin: [7:5]: port number, [4:0]: pin number
}GPIO_InitTypeDef;
...
_LONG_CALL_  HAL_Status HAL_GPIO_IntCtrl_8195aV02(HAL_GPIO_PIN  *GPIO_Pin, uint32_t En);
_LONG_CALL_ uint32_t GPIO_Int_Clear_8195aV02(uint32_t irq_clr);
...
{
    return (int32_t)HAL_GPIO_ReadPin_8195a(GPIO_Pin);    
}
