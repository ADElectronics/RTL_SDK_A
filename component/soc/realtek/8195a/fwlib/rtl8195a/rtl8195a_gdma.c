...
)
{
    ...
    uint32_t MultiBlockCount = pHalGdmaAdapter->MaxMuliBlock;
    uint32_t CtlxLow, CtlxUp, CfgxLow, CfgxUp;
    uint8_t  GdmaIndex = pHalGdmaAdapter->GdmaIndex;
    uint8_t  ChNum = pHalGdmaAdapter->ChNum;
    uint32_t ChEn = pHalGdmaAdapter->ChEn;
    uint8_t  GdmaChIsrBitmap = (ChEn & 0xFF);
    uint8_t  PendingIsrIndex;
