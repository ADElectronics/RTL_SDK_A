...
#define GET_STATUS_HISR(status)				((((*(uint32_t*)status)) & 0x3) |((((*(uint32_t*)status) >> 2) & 0x7) << 17))
#define GET_STATUS_FREE_TX(status)		((((*(uint32_t*)status) >> 5) & 0x7ffffff) << 2)
#define GET_STATUS_RXQ_REQ_LEN(status)	(((*(uint32_t*)((uint8_t *)status + 4))) & 0xffffff)
#define GET_STATUS_TX_SEQ(status)			(((*(uint32_t*)((uint8_t *)status + 4)) >> 24) & 0xff)
