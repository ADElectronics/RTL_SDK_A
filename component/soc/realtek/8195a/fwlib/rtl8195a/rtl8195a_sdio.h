...
typedef struct _SPDIO_HCI_RX_REQ {
	uint8_t HCI_RX_REQ:1;		/* bit[0], CPU trigger this bit to enable SDIO IP RX transfer by fetch BD info */
	uint8_t Reserved:7;			/* bit[7:1], Reserved */
} SPDIO_HCI_RX_REQ, *PSPDIO_HCI_RX_REQ;

// Register REG_SPDIO_CPU_RST_DMA @ 0xBB
typedef struct _SPDIO_CPU_RST_DMA {
	uint8_t Reserved:7;			/* bit[6:0], Reserved */
	uint8_t CPU_RST_SDIO:1;		/* bit[7], CPU set this bit to reset SDIO DMA */
} SPDIO_CPU_RST_DMA, *PSPDIO_CPU_RST_DMA;

// Register REG_SPDIO_CPU_INT_MASK @ 0xC0
typedef struct _SPDIO_CPU_INT_MASK {
	uint16_t TXFIFO_H2C_OVF:1;		/* bit[0], set 0 to mask TXFIFO_H2C_OVF_INT */
	uint16_t H2C_BUS_RES_FAIL:1;		/* bit[1], set 0 to mask H2C_BUS_RES_FAIL_INT */
	uint16_t H2C_DMA_OK:1;			/* bit[2], set 0 to mask H2C_DMA_OK_INT */
	uint16_t C2H_DMA_OK:1;			/* bit[3], set 0 to mask C2H_DMA_OK_INT */
	uint16_t H2C_MSG_INT:1;			/* bit[4], set 0 to mask H2C_MSG_INT_INT */
	uint16_t RPWM_INT:1;				/* bit[5], set 0 to mask RPWM_INT */
	uint16_t RPWM2_INT:1;			/* bit[6], set 0 to mask RPWM2_INT */
	uint16_t SDIO_RST_CMD_INT:1;		/* bit[7], set 0 to mask SDIO_RST_CMD_INT */
	uint16_t BD_FLAG_ERR_INT:1;		/* bit[8], set 0 to mask BD_FLAG_ERR_INT */
	uint16_t Reserved:7;				/* bit[15:9], Reserved */
...
typedef struct _SPDIO_CCPWM {
	uint8_t :1;					/* bit[0] */
	uint8_t WLAN_TRX:1;			/* bit[1], 0: WLAN Off; 1: WLAN On */
	uint8_t RPS_ST:1;			/* bit[2], 0/1: AP Register Sleep/Active state */
	uint8_t WWLAN:1;				/* bit[3], 0/1: "Wake on WLAN"/"Normal" state */
	uint8_t Reserved:3;			/* bit[6:4], Reserved */
	uint8_t TOGGLING:1;			/* bit[7], issue interrupt when 0->1 or 1->0 */
} SPDIO_CCPWM, *PSPDIO_CCPWM;

// Register REG_SPDIO_CPU_IND @ 0xC5
typedef struct _SPDIO_CPU_IND {
	uint8_t SYS_TRX_RDY:1;		/* bit[0], To indicate the Host system that CPU is ready for TRX
...
typedef struct _SPDIO_CPU_H2C_MSG {
	uint32_t CPU_H2C_MSG:30;		/* bit[30:0], Host CPU to FW message, sync from REG_SDIO_H2C_MSG */
	uint32_t Reserved:1;			/* bit[31], Reserved */
} SPDIO_CPU_H2C_MSG, *PSPDIO_CPU_H2C_MSG;

// Register REG_SPDIO_CPU_C2H_MSG @ 0xCC
typedef struct _SPDIO_CPU_C2H_MSG {
	uint32_t CPU_C2H_MSG:30;		/* bit[30:0], FW to Host CPU message, sync to REG_SDIO_C2H_MSG */
	uint32_t Reserved:1;			/* bit[31], Reserved */
...
typedef struct _SPDIO_AHB_DMA_CTRL {
	uint32_t TXFF_WLEVEL:7;		/* bit[6:0], SPDIO TX FIFO water level */
	uint32_t :1;					/* bit[7] */
	uint32_t RXFF_WLEVEL:7;		/* bit[14:8], SPDIO RX FIFO water level */
	uint32_t :1;					/* bit[15] */
	uint32_t AHB_DMA_CS:4;		/* bit[19:16], AHB DMA state */
	uint32_t :1;					/* bit[20] */
	uint32_t AHB_MASTER_RDY:1;	/* bit[21], AHB Master Hready signal */
	uint32_t AHB_DMA_TRANS:2;	/* bit[23:22], AHB DMA Trans value, for debugging */
	uint32_t AHB_BUSY_WAIT_CNT:4;	/* bit[27:24], timeout for AHB controller to wait busy */
	uint32_t AHB_BURST_TYPE:3;	/* bit[30:28], AHB burst type */
	uint32_t DISPATCH_TXAGG:1;	/* bit[31], Enable to dispatch aggregated TX packet */
...
typedef struct _SPDIO_CPU_H2C_MSG {
	uint32_t Reserved:1;			/* bit[31], Reserved */
	uint32_t CPU_H2C_MSG:30;		/* bit[30:0], Host CPU to FW message */
} SPDIO_CPU_H2C_MSG, *PSPDIO_CPU_H2C_MSG;
...
typedef struct _SDIO_TX_BD_ {
	uint32_t	Address;    /* The TX buffer physical address, it must be 4-bytes aligned */
...
typedef struct _SDIO_RX_BD_ {
	uint32_t BuffSize:14;		/* bit[13:0], RX Buffer Size, Maximum 16384-1 */
	...
	uint32_t PhyAddr;			/* The RX buffer physical address, it must be 4-bytes aligned */
} SDIO_RX_BD, *PSDIO_RX_BD;
#else
typedef struct _SDIO_RX_BD_ {
	uint32_t Seq:16;				/* bit[31:16], The sequence number, be used for ?? */
	uint32_t FS:1;					/* bit[15], is the First Segment ? */
	uint32_t LS:1;					/* bit[14], is the Last Segment ? */
	uint32_t BuffSize:14;		/* bit[13:0], RX Buffer Size, Maximum 16384 */
	uint32_t PhyAddr;			/* The RX buffer physical address, it must be 4-bytes aligned */
} SDIO_RX_BD, *PSDIO_RX_BD;
#endif
...
typedef struct _SDIO_TX_BD_BUFFER_ {
		...
		uint8_t TX_Buffer[SDIO_TX_BD_BUF_SIZE];
...
typedef struct _SDIO_RX_BD_BUFFER_ {
		...
		uint8_t RX_Buffer[SDIO_RX_BD_BUF_SIZE];
...
typedef struct _SDIO_TX_PACKET_ {
	...
	uint16_t	PktSize;		// the size (bytes) of this packet
	_LIST list;			// the link list to chain packets
	uint8_t isDyna;          // is Dynamic allocated
...
typedef struct _SDIO_TX_BD_HANDLE_ {
	...
	uint8_t isPktEnd;		// For a packet over 1 BD , this flag to indicate is this BD contains a packet end
	uint8_t isFree;		// is this TX BD free 
...
//	SDIO_RX_DESC RxDesc;	// The RX Descriptor for this packet, to be send to Host ahead this packet
	...
	_LIST list;			// the link list to chain packets
	uint8_t PktBuf[SDIO_RX_BD_BUF_SIZE];       // the Rx_Desc + payload data buffer, the first 24 bytes is reserved for RX_DESC
...
typedef struct _SDIO_RX_BD_HANDLE_ {
	...
	uint8_t isPktEnd;		// For a packet over 1 BD , this flag to indicate is this BD contains a packet end
	uint8_t isFree;		// is this RX BD free (DMA done and its RX packet has been freed)
...
typedef struct _SDIO_MP_RX_PACKET_ {
	...
	uint8_t *pData;          // point to the head of payload of this packet
	uint16_t Offset;        // the offset from the pData to the payload
	uint16_t DataLen;        // the data length of this packet
...
void
MovePKTToRX(
	IN  uint32_t	 Source,  IN  uint32_t  Destination,  IN  uint32_t  PKTSize
);
