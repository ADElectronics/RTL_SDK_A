...
static int INFRA_START_SECTION sdr_test(uint32_t LoopCnt) {
...
#define TapCnt 0x19
	...
	uint32_t reg = HAL_READ32(SYSTEM_CTRL_BASE, REG_SYS_SYSPLL_CTRL1);
	uint32_t faddr = SPI_FLASH_BASE + FLASH_SDRC_PARA_BASE + ((reg & 0x70) >> 1) +  ((reg & BIT17) >> 11) ; // step 8 in FLASH_SDRC_PARA_BASE[64 + 64 bytes]
	...
	if(value.s[0] == 0xFE01 && (value.b[4]^value.b[5]) == 0xFF && (value.b[6]^value.b[7]) == 0xFF) {
		HAL_SDR_WRITE32(REG_SDR_IOCR, (HAL_SDR_READ32(REG_SDR_IOCR) & 0xff) | ((uint32_t)value.b[4] << PCTL_IOCR_RD_PIPE_BFO));
...
//				DBG_8195A("SDR calibration: %02x-%02x-%02x\n", value.b[0], value.b[4], value.b[6]);
		...
		if((*((volatile uint16_t *)(faddr)) & value.s[0]) == value.s[0]
			&& (*((volatile uint32_t *)(faddr + 4)) & value.l[1]) == value.l[1]) {
			*((volatile uint32_t *)(faddr + 4)) = value.l[1];
			DBG_8195A("Write new calibration [%08x] %02x-%02x-%02x\n", faddr, value.b[0], value.b[4], value.b[6]);
		    HalDelayUs(1000);
			*((volatile uint16_t *)(faddr)) = value.s[0];
		} else {
...
static const uint32 tab_seg_def[] = {
...
static uint32 BOOT_RAM_TEXT_SECTION load_segs(uint32 faddr, PIMG2HEAD hdr, uint8 flgload) {
