...
#if !DAC_STATIC_ALLOC
    RtlMfree((uint8_t *)pSalDACMngtAdpt->pUserCB->pTXCB, (sizeof(SAL_DAC_USERCB_ADPT)*SAL_DAC_USER_CB_NUM));
    RtlMfree((uint8_t *)pSalDACMngtAdpt->pIrqGdmaHnd, sizeof(IRQ_HANDLE));
    RtlMfree((uint8_t *)pSalDACMngtAdpt->pHalGdmaOp, sizeof(HAL_GDMA_OP));
    RtlMfree((uint8_t *)pSalDACMngtAdpt->pHalGdmaAdp, sizeof(HAL_GDMA_ADAPTER));
    RtlMfree((uint8_t *)pSalDACMngtAdpt->pDMAConf, sizeof(SAL_DAC_DMA_USER_DEF));
    RtlMfree((uint8_t *)pSalDACMngtAdpt->pUserCB, sizeof(SAL_DAC_USER_CB));
    RtlMfree((uint8_t *)pSalDACMngtAdpt->pIrqHnd, sizeof(IRQ_HANDLE));
    RtlMfree((uint8_t *)pSalDACMngtAdpt->pHalOp, sizeof(HAL_DAC_OP));
    RtlMfree((uint8_t *)pSalDACMngtAdpt->pHalInitDat, sizeof(HAL_DAC_INIT_DAT));
    RtlMfree((uint8_t *)pSalDACMngtAdpt->pSalHndPriv, sizeof(SAL_DAC_HND_PRIV));    
    RtlMfree((uint8_t *)pSalDACMngtAdpt, sizeof(SAL_DAC_MNGT_ADPT));
#else
    ;
#endif
...
DACGDMALLPISRHandle(
    ...
    uint8_t  IsrTypeMap, BlockIndex;
    uint8_t *pSrc = NULL, *pDst = NULL;
...
RtkDACIrqInit(
    ...
    switch (pSalDACHND->DevNum){
        ...
        case DAC0_SEL:
        {
            pIrqHandle->Data = (uint32_t) (pSalDACHND);
...
#if DAC_DMA_OP_TYPE
const uint16_t DACDmaChNo[10] = {GdmaNoCh   ,GdmaCh0,
                             GdmaCh1    ,GdmaCh2,
                             GdmaCh3    ,GdmaCh4,
                             GdmaCh5    ,GdmaCh6,
                             GdmaCh7    ,GdmaAllCh};
...
RtkDACDMAInit(
    ...
    pIrqHandleDACGdma->Data     = (uint32_t) (pSalDACHND);  
...
RtkDACSend(
    ...
    switch (pSalDACHND->DACInType) {
        ...
        case DAC_INPUT_DMA_ONEBLK:
        {
            ...
            pHALDACGdmaAdpt->ChSar              =   (uint32_t)pSalDACHND->pTXBuf->pDataBuf;
            pHALDACGdmaAdpt->ChDar              =   (uint32_t)(DAC_REG_BASE+(pSalDACHND->DevNum*0x800));  
...
RtkDACLoadDefault(
    ...
    pSalDACHND->DACExd          =   0;
    pSalDACHND->ErrType         =   (uint32_t)NULL;    
