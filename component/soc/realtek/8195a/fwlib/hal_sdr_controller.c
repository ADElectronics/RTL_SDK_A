...
int32_t MemTest(uint32_t loop_cnt);
uint32_t SdrCalibration(void);
//uint32_t Sdr_Rand2(void);
...
//HAL_CUT_B_RAM_DATA_SECTION
extern uint32_t  AvaWds[2][REC_NUM];
#endif
#else
typedef struct {
	uint32_t m[2][REC_NUM];
} sAvaWds, * pAvaWds;
#endif
...
int
rt_rpt_h_addr(uint8_t rpt)
{
    uint32_t r_val, offset;
...
int
rt_txrpt_read32(uint8_t macid, uint8_t offset)
{
    uint32_t r_val;
...
int
rt_txrpt_read16(uint8_t macid, uint8_t offset)
{
    uint16_t r_val;
...
int
rt_txrpt_read8(uint8_t macid, uint8_t offset)
{
    uint8_t r_val;
...
int
rt_txrpt_read_1b(uint8_t macid, uint8_t offset, uint8_t bit_offset)
{
    uint8_t r_val = ((rt_txrpt_read8(macid, offset) & BIT(bit_offset))?1:0);
...
int
rt_txrpt_write32(uint8_t macid, uint8_t offset, uint32_t val)
{
    rt_rpt_h_addr(TXRPT);
...
int
rt_txrpt_write16(uint8_t macid, uint8_t offset, uint16_t val)
{
    rt_rpt_h_addr(TXRPT);
...
int
rt_txrpt_write8(uint8_t macid, uint8_t offset, uint8_t val)
{
    rt_rpt_h_addr(TXRPT);
...
int
rt_txrpt_write_1b(uint8_t macid, uint8_t offset, uint8_t bit_offset, uint8_t val)
{
    uint8_t r_val = rt_txrpt_read8(macid, offset);
...
)
{
    ...
    uint32_t Loop, LoopIndex, Value32, Addr, Loop1, LoopIndex1;
...
{
	//DBG_8195A("%s(%p)\n", __func__, DramInfo);
    uint32_t CsBstLen = 0;            // 0:bst_4, 1:bst_8
    uint32_t CasWr = 0;//, CasWrT;       // cas write latency
    uint32_t CasRd = 0, CasRdT = 0, CrlSrt = 0;  // cas read latency
    uint32_t AddLat;
    uint32_t DramEmr2 = 0, DramMr0 = 0;
    uint32_t CrTwr, DramMaxWr, DramWr;
    uint32_t CrTrtw = 0, CrTrtwT = 0;
    uint32_t DrmaPeriod; 
    ...
    DfiRate = 1 << (uint32_t) (DramInfo->DfiRate);
...
#endif
	DBG_8195A("%s()\n", __func__);
    uint32_t RdPipe = 0, TapCnt = 0, Pass = 0, AvaWdsCnt = 0;
    uint32_t RdPipeCounter, RecNum[2], RecRdPipe[2];//, AvaWds[2][REC_NUM];
...
//    BOOL PassFlag = 0;
	...
	uint32_t valid;
	union { uint8_t b[4]; uint32_t l;} value;
////
...
////
	...
	uint32_t CpuType = ((valid & (0x70)) >> 4) + ((valid & BIT17) >> 17) * 8;
	...
	pAvaWds AvaWds = (pAvaWds) tcm_heap_calloc(sizeof(uint32_t)*REC_NUM*2);
#else
    _memset((uint8_t*)AvaWds, 0, sizeof(AvaWds));
#endif
#else
    uint32_t  AvaWds[2][REC_NUM];
    _memset((uint8_t*)AvaWds, 0, sizeof(uint32_t)*REC_NUM*2);
#endif
...
//                }
        ...
        if (RdPipeCounter > 2) {
            uint8_t BestRangeIndex, BestIndex;
...
//            WR_DATA(SDR_CLK_DLY_CTRL, Value32);
			...
			value.b[0] = (uint8_t)RdPipe;
			value.b[1] = ~value.b[0];
			value.b[2] = (uint8_t)TapCnt;
			value.b[3] = ~value.b[2];
#if DEBUG_SDRAM > 1
...
HAL_SDRC_TEXT_SECTION uint32_t SdrControllerInit( void)
{
	DBG_SDR_ERR("No SDRAM!\n");
	SdrPowerOff();
	return 1;
