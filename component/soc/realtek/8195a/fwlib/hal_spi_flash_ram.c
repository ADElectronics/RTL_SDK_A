...
#define WR_DATA(addr, data)    (*((volatile uint32_t*)(addr)) = (data))      
#define RD_DATA(addr)          (*((volatile uint32_t*)(addr)))

BOOLEAN SpicFlashInitRtl8195A(uint8_t SpicBitMode);
uint32_t SpicCalibrationRtl8195A(uint8_t SpicBitMode, uint32_t DefRdDummyCycle);          // spi-flash calibration

void SpicSetFlashStatusRefinedRtl8195A(uint32_t data, SPIC_INIT_PARA SpicInitPara);

void SpicConfigAutoModeRtl8195A(uint8_t SpicBitMode);      // config spi-flash controller to auto mode
void SpicReadIDRtl8195A(void);


_LONG_CALL_
extern void SpicInitRtl8195A(uint8_t InitBaudRate, uint8_t SpicBitMode);            // spi-flash controller initialization

_LONG_CALL_
extern void SpicRxCmdRtl8195A(uint8_t);         // recieve command

_LONG_CALL_
extern void SpicSetFlashStatusRtl8195A(uint32_t data, SPIC_INIT_PARA SpicInitPara);       // WRSR, write spi-flash status register
...
_LONG_CALL_
extern uint32_t SpicCmpDataForCalibrationRtl8195A(void);            // compare read_data and golden_data
#ifdef CONFIG_FPGA 
_LONG_CALL_
...
_LONG_CALL_ 
extern uint8_t SpicGetFlashStatusRtl8195A(SPIC_INIT_PARA SpicInitPara);

_LONG_CALL_
extern void SpicTxCmdRtl8195A(uint8_t cmd, SPIC_INIT_PARA SpicInitPara);

struct ava_window {
    uint16_t baud_rate;
    uint16_t auto_length;
    uint32_t dly_line_sp;
    uint32_t dly_line_ep;
};
...
)
{
    ...
    for (i=0;i<DataPhaseLen;i++) {
        HAL_SPI_WRITE8(REG_SPIC_DR0, (uint8_t)*(pData+i));
    }
...
)
{
    ...
    if ((HAL_READ32(SYSTEM_CTRL_BASE,REG_SYS_EFUSE_SYSCFG6) & BIT(25))&&
        ((((uint8_t)HAL_READ32(SYSTEM_CTRL_BASE,REG_SYS_SYSTEM_CFG0))>>4)>= 2)){
...
(
    ...
    IN  SPIC_INIT_PARA SpicInitPara,
    IN  uint32_t addr,
    IN  uint32_t* LengthInfo
)
{

    uint32_t Info;
    uint32_t Length = (uint32_t) *LengthInfo;
    uint32_t OccuSize;
    uint32_t writeword;
    uint32_t lastwriteword;
    uint32_t ProgramLength;
    uint32_t instruction;
    uint32_t PageSize;
    uint8_t addrbyte[3];
    uint8_t UnalignOffset;
    uint8_t lastunalignoffset;
    uint8_t index;
    uint8_t *ptr;
    uint8_t *buff;
    ...
    *LengthInfo -= ProgramLength;
    ...
    if(addr & 0x03){
        ...
        ptr = (uint8_t*) &writeword + UnalignOffset;
        ...
        if(ProgramLength >= 4){
            writeword = (uint32_t)(*buff) | (uint32_t)((*(buff+1)) << 8)|(uint32_t)((*(buff+2)) <<16)|(uint32_t)((*(buff+3))<<24);
        }
    }
//address already align
    ...
    while(ProgramLength > 4){
        if((uint32_t)buff & 0x03){
            ...
            //while(ProgramLength >= 4){
                HAL_SPI_WRITE32(REG_SPIC_DR0, (uint32_t)*((uint32_t *)buff)); 
                ProgramLength -=4;
                buff+=4;
            //}        
        }
...
) 
{
    ...
    uint32_t rd_data, baudr, autolen, dly_line;
    uint32_t total_ava_wds=0;
    uint32_t tmp_str_pt, tmp_end_pt, pass, last_pass;
...
/**
  ...
  *
  * @param  IN  uint8_t cmd: flash RX command
  *
  * @retval NA
  */
HAL_FLASH_TEXT_SECTION
...
SpicRxCmdRefinedRtl8195A(
    ...
    uint32_t RdDummyCycle;
    uint32_t BaudRate;
    uint32_t BaudRate12bit;
    uint32_t DelayLine;
    uint32_t DelayLine8bit;

    uint32_t AutoLength = 0;
    uint8_t  CpuClk = ((HAL_READ32(SYSTEM_CTRL_BASE, REG_SYS_CLK_CTRL1) & (0x70)) >> 4);
...
/**
  ...
  * @param  IN  SPIC_INIT_PARA SpicInitPara: spic init parameters with timing setting
  *
  * @retval uint8_t flash status register value
  */
HAL_FLASH_TEXT_SECTION
uint8_t 
...
SpicNVMCalLoad(uint8_t BitMode, uint8_t CpuClk)
{
    SPIC_INIT_PARA *pspci_para;
    uint32_t spci_para;
    uint32_t spci_para_inv;
...
/**
  ...
  */
HAL_FLASH_TEXT_SECTION void
SpicNVMCalStore(uint8_t BitMode, uint8_t CpuClk)
{
    SPIC_INIT_PARA *pspci_para;
    uint32_t spci_para;
    uint32_t flash_offset;
