...
static volatile uint32_t ADCDatBuf[2];
static volatile uint8_t  ADCFullStsFlag;
static RTK_STATUS
RtkADCPinMuxDeInit(
...
/* Global variables */
uint8_t  SalAdcInitialFlag = 0;
#ifdef CONFIG_SOC_PS_MODULE
uint8_t  SalAdcEnableState = 0;
#endif
...
#ifdef CONFIG_KERNEL
#if 1 // pvvx
	RtlMfree((uint8_t *)pSalADCMngtAdpt,
			sizealign4(SAL_ADC_MNGT_ADPT)
    		+ sizealign4(SAL_ADC_HND_PRIV)
			+ sizealign4(HAL_ADC_INIT_DAT)
			+ sizealign4(HAL_ADC_OP)
			...
			+ (sizealign4(SAL_ADC_USERCB_ADPT)*SAL_ADC_USER_CB_NUM));
#else
    RtlMfree((uint8_t *)pSalADCMngtAdpt->pUserCB->pTXCB, (sizeof(SAL_ADC_USERCB_ADPT)*SAL_ADC_USER_CB_NUM));
    RtlMfree((uint8_t *)pSalADCMngtAdpt->pIrqGdmaHnd, sizeof(IRQ_HANDLE));
    RtlMfree((uint8_t *)pSalADCMngtAdpt->pHalGdmaOp, sizeof(HAL_GDMA_OP));
    RtlMfree((uint8_t *)pSalADCMngtAdpt->pHalGdmaAdp, sizeof(HAL_GDMA_ADAPTER));
    RtlMfree((uint8_t *)pSalADCMngtAdpt->pUserCB, sizeof(SAL_ADC_USER_CB));
    RtlMfree((uint8_t *)pSalADCMngtAdpt->pIrqHnd, sizeof(IRQ_HANDLE));
    RtlMfree((uint8_t *)pSalADCMngtAdpt->pHalOp, sizeof(HAL_ADC_OP));
    RtlMfree((uint8_t *)pSalADCMngtAdpt->pHalInitDat, sizeof(HAL_ADC_INIT_DAT));
    RtlMfree((uint8_t *)pSalADCMngtAdpt->pSalHndPriv, sizeof(SAL_ADC_HND_PRIV));    
    RtlMfree((uint8_t *)pSalADCMngtAdpt, sizeof(SAL_ADC_MNGT_ADPT));
#endif
#else
    ;
#endif
...
//    pSalADCUserCB   = pSalADCHND->pUserCB;
    ...
    if (pSalADCHND->OpType == ADC_RDREG_TYPE){
        ADCFullStsFlag = 1;
        ADCDatBuf[0]    = (uint32_t)HAL_ADC_READ32(REG_ADC_FIFO_READ);
        ADCDatBuf[1]    = (uint32_t)HAL_ADC_READ32(REG_ADC_FIFO_READ);
...
*/
    ...
    pIrqHandle->Data        =   (uint32_t)(pSalADCHND);
...
*/
    ...
    pIrqHandleADCGdma->Data     = (uint32_t) (pSalADCHND);  
...
RtkADCReceive(
    ...
    if (pSalADCHND->OpType == ADC_DMA_TYPE) {
        ...
        pHALADCGdmaAdpt->ChSar              =   (uint32_t)(ADC_REG_BASE);
        pHALADCGdmaAdpt->ChDar              =   (uint32_t)pSalADCHND->pRXBuf->pDataBuf;
        pHALADCGdmaAdpt->MuliBlockCunt      = 0;
...
//extern uint32_t HalDelayUs(IN  uint32_t us);
...
RtkADCReceiveBuf(
    ...
    /* Clear ADC Status */
	//HAL_ADC_READ32(REG_ADC_INTR_STS);
	AdcTempDat  = (uint32_t)HAL_ADC_READ32(REG_ADC_INTR_STS);
...
///    HalDelayUs(2000); ?
    ...
    pHalADCOP->HalADCIntrCtrl(pSalADCHND->pInitDat);
	pSalADCHND->DevSts = ADC_STS_IDLE;
	AdcTempDat  = (uint32_t)HAL_ADC_READ32(REG_ADC_ANAPAR_AD0);

	if ((AdcTempDat & BIT_ADC_EN_MANUAL) == 0){
        AdcTempDat  = (uint32_t)HAL_ADC_READ32(REG_ADC_ANAPAR_AD1);
    ...
    *pBuf       = (uint32_t)ADCDatBuf[0];
    *(pBuf+1)   = (uint32_t)ADCDatBuf[1];
    ADCDatBuf[0] = 0;
    ADCDatBuf[1] = 0;
...
RtkADCLoadDefault(
    ...
    pSalADCHND->ADCExd          =   0;
    pSalADCHND->ErrType         =   (uint32_t)NULL;
...
RtkADCDisablePS(
    ...
    PSAL_ADC_HND    pSalADCHND  =   (PSAL_ADC_HND) Data;
    uint8_t adcIdx = pSalADCHND->DevNum;
#ifdef CONFIG_SOC_PS_MODULE
    REG_POWER_STATE adcPwrState;
#endif
