...
I2CISRHandle_Patch(
    ...
    uint32_t I2CLocalTemp    = 0;
    uint32_t I2CInTOTcnt     = 0;
    uint32_t I2CIrqIdx       = 0;
    uint32_t InTimeoutCount  = 0;
    uint32_t InStartCount    = 0;
    volatile uint32_t I2CLocalRawSts  = 0;
    uint32_t I2CStsTmp       = 0;
    ...
    if (pHalI2COP->HalI2CReadReg(pHalI2CInitDat,REG_DW_I2C_IC_INTR_STAT) & 
        ...
        if (pSalI2CHND->I2CMaster == I2C_MASTER_MODE){
            ...
            if (pSalI2CHND->pRXBuf->DataLen > 0) {
                ...
                I2CInTOTcnt     = (uint32_t)pSalI2CMngtAdpt->InnerTimeOut;
                InTimeoutCount  = 0;
                ...
                while (1) {                  
                    I2CLocalRawSts = (uint32_t)pHalI2COP->HalI2CReadReg(pHalI2CInitDat,REG_DW_I2C_IC_STATUS);
...
RtkI2CSend_Patch(
    ...
    uint32_t InTimeoutCount = 0;
    uint32_t InStartCount = 0;
    uint32_t I2CChkRawSts = 0;
    uint32_t I2CChkRawSts2 = 0;
    uint32_t I2CDataLenBak = 0;
    uint32_t I2CDataPtrBak = 0;
    //uint32_t I2CInTOTcntIntr = 0;
    //uint32_t InTimeoutCountIntr = 0;
    //uint32_t InStartCountIntr = 0;
    uint32_t I2CMtrTtyCnt    = 0;
    ...
    if (pSalI2CHND->I2CMaster == I2C_MASTER_MODE) {   
        ...
        if (pSalI2CHND->OpType == I2C_POLL_TYPE) { /* if (pSalI2CHND->OpType == I2C_POLL_TYPE) */
            ...
            for (;pSalI2CHND->pTXBuf->DataLen>0;) {
				...
				if (pSalI2CHND->I2CExd & I2C_EXD_MTR_ADDR_RTY) {
                    ...
                    I2CLocalTemp = (uint32_t)pHalI2COP->HalI2CReadReg(pHalI2CInitDat,REG_DW_I2C_IC_RAW_INTR_STAT);
	                if (I2CLocalTemp & BIT_IC_RAW_INTR_STAT_TX_ABRT) {
                        I2CMtrTtyCnt++;
                        I2CLocalTemp = (uint32_t)pHalI2COP->HalI2CReadReg(pHalI2CInitDat,REG_DW_I2C_IC_TX_ABRT_SOURCE);
        ...
        if (pSalI2CHND->OpType == I2C_INTR_TYPE) {  /* if (pSalI2CHND->OpType == I2C_INTR_TYPE) */
            ...
            I2CDataLenBak = (uint32_t)(pSalI2CHND->pTXBuf->DataLen);
            I2CDataPtrBak = (uint32_t)(pSalI2CHND->pTXBuf->pDataBuf);
            /* I2C Device Status Update */
            pSalI2CHND->DevSts = I2C_STS_IDLE;
            pHalI2CInitDat->I2CIntrClr = REG_DW_I2C_IC_CLR_TX_ABRT;
            pHalI2COP->HalI2CClrIntr(pHalI2CInitDat);
...
SEND_I2C_WR_CMD_INTR:
				...
				if (pSalI2CHND->I2CExd & I2C_EXD_MTR_ADDR_RTY) {
                    uint32_t I2CInTOTcntRty      = 0;
                    uint32_t InTimeoutCountRty   = 0;
                    uint32_t InStartCountRty     = 0;
                    ...
                    HalDelayUs((uint32_t)((1000*30)/pHalI2CInitDat->I2CClk));    //the 10 is for ten bit time                  
                    
                    I2CChkRawSts = (uint32_t)pHalI2COP->HalI2CReadReg(pHalI2CInitDat,REG_DW_I2C_IC_RAW_INTR_STAT);
	                if (I2CChkRawSts & BIT_IC_RAW_INTR_STAT_TX_ABRT) {
	                    I2CMtrTtyCnt++;
	                    I2CChkRawSts = (uint32_t)pHalI2COP->HalI2CReadReg(pHalI2CInitDat,REG_DW_I2C_IC_TX_ABRT_SOURCE);
	                    if (I2CMtrTtyCnt > I2C_MTR_RTY_CNT) {
                        ...
                        I2CChkRawSts = (uint32_t)pHalI2COP->HalI2CReadReg(pHalI2CInitDat,REG_DW_I2C_IC_ENABLE_STATUS);
                        ...
                        pSalI2CHND->pTXBuf->DataLen = (uint16_t)I2CDataLenBak;
                        pSalI2CHND->pTXBuf->pDataBuf= (uint8_t*)I2CDataPtrBak;
                        /* Calculate user time out parameters */
            			InTimeoutCount = 0;
            			InStartCount = 0;
                        I2CInTOTcnt = pSalI2CHND->TimeOut;          
	                ...
	                else if (((uint32_t)pHalI2COP->HalI2CReadReg(pHalI2CInitDat,REG_DW_I2C_IC_STATUS) & BIT_IC_STATUS_TFE) != BIT_IC_STATUS_TFE) {
                        I2CMtrTtyCnt++;
                        I2CChkRawSts = (uint32_t)pHalI2COP->HalI2CReadReg(pHalI2CInitDat,REG_DW_I2C_IC_TX_ABRT_SOURCE);
	                    if (I2CMtrTtyCnt > I2C_MTR_RTY_CNT) {
                            if ((I2CChkRawSts & (BIT_IC_TX_ABRT_SOURCE_ABRT_7B_ADDR_NOACK | 
        ...
        if (pSalI2CHND->OpType == I2C_DMA_TYPE) {  
            ...
            pHalI2CTxGdmaAdpt->ChSar                = (uint32_t)pSalI2CHND->pTXBuf->pDataBuf;
            pHalI2CTxGdmaAdpt->ChDar                = (uint32_t)(I2C0_REG_BASE+REG_DW_I2C_IC_DATA_CMD+
...
SEND_I2C_RD_CMD:
        ...
        if (pSalI2CHND->OpType == I2C_INTR_TYPE) {
            ...
            I2CDataLenBak = (uint32_t)(pSalI2CHND->pRXBuf->DataLen);
            I2CDataPtrBak = (uint32_t)(pSalI2CHND->pRXBuf->pDataBuf);
            ...
            /* Clear RX FIFO */
            I2CChkRawSts = (uint32_t)pHalI2COP->HalI2CReadReg(pHalI2CInitDat,REG_DW_I2C_IC_RXFLR);
...
SEND_I2C_RD_CMD_INTR:
                ...
                if (pSalI2CHND->I2CExd & I2C_EXD_MTR_ADDR_RTY) {
                    ...
                    I2CChkRawSts = (uint32_t)pHalI2COP->HalI2CReadReg(pHalI2CInitDat,REG_DW_I2C_IC_RAW_INTR_STAT);
                    I2CChkRawSts2 = (uint32_t)pHalI2COP->HalI2CReadReg(pHalI2CInitDat,REG_DW_I2C_IC_STATUS);
	                if (I2CChkRawSts & BIT_IC_RAW_INTR_STAT_TX_ABRT) {
                        I2CMtrTtyCnt++;
	                    I2CChkRawSts = (uint32_t)pHalI2COP->HalI2CReadReg(pHalI2CInitDat,REG_DW_I2C_IC_TX_ABRT_SOURCE);
	                    if (I2CMtrTtyCnt > I2C_MTR_RTY_CNT) {
                        ...
                        pSalI2CHND->DevSts  = I2C_STS_RX_READY;
                        pSalI2CHND->ErrType = 0;
                        pSalI2CHND->pRXBuf->DataLen = (uint16_t)I2CDataLenBak;
                        pSalI2CHND->pRXBuf->pDataBuf= (uint8_t *)I2CDataPtrBak;
    ...
    else
    {
        ...
        if (pSalI2CHND->OpType == I2C_DMA_TYPE) {
            ...
            pHalI2CRxGdmaAdpt->ChSar                = (uint32_t)(I2C0_REG_BASE+REG_DW_I2C_IC_DATA_CMD+
                                                                pSalI2CHND->DevNum*0x400);
            pHalI2CRxGdmaAdpt->ChDar                = (uint32_t)pSalI2CHND->pRXBuf->pDataBuf;
...
RtkI2CDisablePS(
    ...
    PSAL_I2C_HND    pSalI2CHND  =   (PSAL_I2C_HND) Data;
    uint8_t i2cIdx = pSalI2CHND->DevNum;
#ifdef CONFIG_SOC_PS_MODULE
    REG_POWER_STATE i2cPwrState;
#endif
...
#ifdef I2C_STATIC_ALLOC
    RtlMfree((uint8_t *)pSalI2CMngtAdpt->pUserCB->pTXCB, (sizeof(SAL_I2C_USERCB_ADPT)*SAL_USER_CB_NUM));
    RtlMfree((uint8_t *)pSalI2CMngtAdpt->pDMAConf, (sizeof(SAL_I2C_DMA_USER_DEF)));
    RtlMfree((uint8_t *)pSalI2CMngtAdpt->pIrqRxGdmaHnd, (sizeof(IRQ_HANDLE)));
    RtlMfree((uint8_t *)pSalI2CMngtAdpt->pIrqTxGdmaHnd, (sizeof(IRQ_HANDLE)));
    RtlMfree((uint8_t *)pSalI2CMngtAdpt->pHalGdmaOp, (sizeof(HAL_GDMA_OP)));
    RtlMfree((uint8_t *)pSalI2CMngtAdpt->pHalRxGdmaAdp, (sizeof(HAL_GDMA_ADAPTER)));
    RtlMfree((uint8_t *)pSalI2CMngtAdpt->pHalTxGdmaAdp, (sizeof(HAL_GDMA_ADAPTER)));
    RtlMfree((uint8_t *)pSalI2CMngtAdpt->pUserCB, sizeof(SAL_I2C_USER_CB));
    RtlMfree((uint8_t *)pSalI2CMngtAdpt->pIrqHnd, sizeof(IRQ_HANDLE));
    RtlMfree((uint8_t *)pSalI2CMngtAdpt->pHalOp, sizeof(HAL_I2C_OP));
    RtlMfree((uint8_t *)pSalI2CMngtAdpt->pHalInitDat, sizeof(HAL_I2C_INIT_DAT));
    RtlMfree((uint8_t *)pSalI2CMngtAdpt->pSalHndPriv, sizeof(SAL_I2C_HND_PRIV));    
    RtlMfree((uint8_t *)pSalI2CMngtAdpt, sizeof(SAL_I2C_MNGT_ADPT));
#else
    ;
#endif
