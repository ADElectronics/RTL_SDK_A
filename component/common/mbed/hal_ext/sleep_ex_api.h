...
typedef struct _SLEEP_WKUP_EVENT_ {
    uint8_t wakeup_event;    // Wake up event: Timer, NFC, GPIO
    uint8_t gpio_option;     // GPIO Wakeup setting: [3:0]: Pin 3~0 enable, [7:4]: pin3~0 active high/low
    uint32_t timer_duration; // the sleep duration and then wakeup
