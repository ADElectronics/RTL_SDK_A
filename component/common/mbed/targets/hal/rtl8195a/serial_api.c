...
static void SerialTxDoneCallBack(void *pAdapter)
{
    ...
    uint8_t uart_idx = pHalRuartAdapter->UartIndex;
...
int32_t serial_recv_blocked (serial_t *obj, char *prxbuf, uint32_t len, uint32_t timeout_ms)
{
    ...
    ret = pHalRuartOp->HalRuartRecv(pHalRuartAdapter, (uint8_t*)prxbuf, len, timeout_ms);
...
int32_t serial_send_blocked (serial_t *obj, char *ptxbuf, uint32_t len, uint32_t timeout_ms)
{
    ...
    pHalRuartOp = &(obj->hal_uart_op);
    obj->tx_len = len;
    ret = pHalRuartOp->HalRuartSend(pHalRuartAdapter, (uint8_t*)ptxbuf, len, timeout_ms);
    return (ret);
}
...
int32_t serial_recv_stream (serial_t *obj, char *prxbuf, uint32_t len)
{
    ...
    pHalRuartOp = &(obj->hal_uart_op);
    obj->rx_len = len;
    ret = pHalRuartOp->HalRuartIntRecv(pHalRuartAdapter, (uint8_t*)prxbuf, len);
    return (ret);
}
...
int32_t serial_send_stream (serial_t *obj, char *ptxbuf, uint32_t len)
{
    ...
    obj->tx_len = len;
    HalRuartEnterCritical(pHalRuartAdapter);
    ret = pHalRuartOp->HalRuartIntSend(pHalRuartAdapter, (uint8_t*)ptxbuf, len);
    HalRuartExitCritical(pHalRuartAdapter);
    return (ret);
}
...
//    pHalRuartOp = &(obj->hal_uart_op);
    ...
    obj->rx_len = len;
    HalRuartEnterCritical(pHalRuartAdapter);
    ret = HalRuartDmaRecv(pHalRuartAdapter, (uint8_t*)prxbuf, len);
    HalRuartExitCritical(pHalRuartAdapter);
    return (ret);
}
...
//    pHalRuartOp = &(obj->hal_uart_op);
    ...
    obj->tx_len = len;
    HalRuartEnterCritical(pHalRuartAdapter);
    ret = HalRuartDmaSend(pHalRuartAdapter, (uint8_t*)ptxbuf, len);
    HalRuartExitCritical(pHalRuartAdapter);
    return (ret);
}
