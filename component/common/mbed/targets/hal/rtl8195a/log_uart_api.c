...
const uint32_t log_uart_support_rate[] = {
...
extern HAL_Status RuartIsTimeout (uint32_t StartCount, uint32_t TimeoutCnt);
extern uint32_t HalLogUartInitSetting(HAL_LOG_UART_ADAPTER *pUartAdapter);
...
void log_uart_line_status_handler(log_uart_t *obj, void *handler, uint32_t id) 
{
    ...
    pUartAdapter->LineStatusCallback = (void(*)(void*, uint8_t))handler;
...
int32_t log_uart_recv_stream (log_uart_t *obj, char *prxbuf, uint32_t len)
{
    ...
    ret = (int)HalLogUartIntRecv(pUartAdapter, (uint8_t*)prxbuf, len);
    return (ret);
}
...
int32_t log_uart_send_stream (log_uart_t *obj, char *ptxbuf, uint32_t len)
{
    ...
    ret = (int)HalLogUartIntSend(pUartAdapter, (uint8_t*)ptxbuf, len);
    return (ret);
}
...
int32_t log_uart_send_stream_abort (log_uart_t *obj)
{
    ...
    ret = (uint32_t)pUartAdapter->pTxBuf - (uint32_t)pUartAdapter->pTxStartAddr;
    return (ret);
}
...
int32_t log_uart_recv_stream_abort (log_uart_t *obj)
{
    ...
    ret = (uint32_t)pUartAdapter->pRxBuf - (uint32_t)pUartAdapter->pRxStartAddr;
    return (ret);
}
