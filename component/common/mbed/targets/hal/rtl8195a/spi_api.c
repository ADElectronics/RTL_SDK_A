...
extern void HAL_GPIO_PullCtrl(uint32_t pin, uint32_t mode);
...
void spi_format (spi_t *obj, int bits, int mode, int slave)
{
    ...
    if (pHalSsiAdaptor->Role == SSI_SLAVE) {
        if (pHalSsiAdaptor->SclkPolarity == SCPOL_INACTIVE_IS_LOW) {
            HAL_GPIO_PullCtrl((uint32_t)obj->sclk, hal_PullDown);
        }
        else {
            HAL_GPIO_PullCtrl((uint32_t)obj->sclk, hal_PullUp);
        }
    }
#endif
...
void spi_frequency (spi_t *obj, int hz)
{
    ...
    pHalSsiAdaptor = &obj->spi_adp;
    HalSsiSetSclk(pHalSsiAdaptor, (uint32_t)hz);
}
...
int32_t spi_slave_write_stream(spi_t *obj, char *tx_buffer, uint32_t length)
{
    ...
    if ((ret=pHalSsiOp->HalSsiWriteInterrupt(pHalSsiAdaptor, (uint8_t *) tx_buffer, length)) != HAL_OK) {
        obj->state &= ~SPI_STATE_TX_BUSY;
    }
    return ret;
}
...
int32_t spi_slave_read_stream_dma(spi_t *obj, char *rx_buffer, uint32_t length)
{
    ...
    obj->state |= SPI_STATE_RX_BUSY;
    ret = HalSsiDmaRecv(pHalSsiAdaptor, (uint8_t *) rx_buffer, length);
    if (ret != HAL_OK) {
        obj->state &= ~SPI_STATE_RX_BUSY;
    }
    return (ret);
...
int32_t spi_slave_write_stream_dma(spi_t *obj, char *tx_buffer, uint32_t length)
{
    ...
    obj->state |= SPI_STATE_TX_BUSY;
    ret = HalSsiDmaSend(pHalSsiAdaptor, (uint8_t *) tx_buffer, length);
    if (ret != HAL_OK) {
        obj->state &= ~SPI_STATE_TX_BUSY;
    }
    return (ret);
        ...
        char *rx_buffer, uint32_t length)
{
    ...
    obj->state |= SPI_STATE_RX_BUSY;
    /* as Master mode, sending data will receive data at sametime */
    if ((ret=HalSsiDmaRecv(pHalSsiAdaptor, (uint8_t *) rx_buffer, length)) == HAL_OK) {
        obj->state |= SPI_STATE_TX_BUSY;
        if ((ret=HalSsiDmaSend(pHalSsiAdaptor, (uint8_t *) tx_buffer, length)) != HAL_OK) {
            obj->state &= ~(SPI_STATE_RX_BUSY|SPI_STATE_TX_BUSY);
        }
    }
    else {
...
int32_t spi_master_read_stream_dma(spi_t *obj, char *rx_buffer, uint32_t length)
{
    ...
    if (obj->dma_en & SPI_DMA_TX_EN) {
        ...
        ret = HalSsiDmaSend(pHalSsiAdaptor, (uint8_t *) rx_buffer, length);
        if (ret != HAL_OK) {
            obj->state &= ~SPI_STATE_RX_BUSY;
        }
    }
