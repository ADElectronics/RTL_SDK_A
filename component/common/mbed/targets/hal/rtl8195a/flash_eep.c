...
//-----------------------------------------------------------------------------
static FLASH_EEP_ATTR unsigned int get_addr_bscfg(bool flg)
{
...
// ret < FMEM_ERROR_MAX - ошибка
//-----------------------------------------------------------------------------
static FLASH_EEP_ATTR unsigned int get_addr_fobj(unsigned int base, fobj_head *obj, bool flg)
{
//	if(base == 0) return 0;
	fobj_head fobj;
...
//-----------------------------------------------------------------------------
static FLASH_EEP_ATTR unsigned int get_addr_fobj_save(unsigned int base, fobj_head obj)
{
	fobj_head fobj;
	unsigned int faddr = base + 4;
...
// Returns      : адрес для записи объекта
//-----------------------------------------------------------------------------
static FLASH_EEP_ATTR unsigned int pack_cfg_fmem(fobj_head obj)
{
	fobj_head fobj;
...
#else
				SpicUserReadFourByteRtl8195A(len, rdaddr, (uint32_t *)pbuf, flashobj.SpicInitPara.Mode.BitMode);
#endif
				int i = 0;
				int size4b = len >> 2;
...
//-----------------------------------------------------------------------------
static signed short FLASH_EEP_ATTR _flash_write_cfg(void *ptr, unsigned short id, unsigned short size)
{
	fobj_head fobj;
	fobj.n.id = id;
	fobj.n.size = size;
//	bool retb = false;
	...
	if(len) SpicUserProgramRtl8195A((uint8_t *)ptr, 1, faddr, len);
#else
