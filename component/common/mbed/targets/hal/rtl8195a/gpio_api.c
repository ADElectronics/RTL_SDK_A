...
const uint8_t GPIO_SWPORT_DR_TBL[] = {
    GPIO_PORTA_DR,
    GPIO_PORTB_DR,
    GPIO_PORTC_DR
};

const uint8_t GPIO_EXT_PORT_TBL[] = {
    GPIO_EXT_PORTA,
    GPIO_EXT_PORTB,
    GPIO_EXT_PORTC
};
...
uint32_t gpio_set(PinName pin) 
{
    ...
    pin_function(pin, 0);
    ip_pin = HAL_GPIO_GetPinName((uint32_t)pin);
...
void gpio_init(gpio_t *obj, PinName pin) 
{
	...
	gpio_set_hal_pin_mode(obj); // добавление для быстрой инициализации
...
//    obj->mode = pull_type;
//    gpio_set_hal_pin_mode(obj);    
    HAL_GPIO_PullCtrl((uint32_t) obj->pin, (uint32_t)pull_type);
}
