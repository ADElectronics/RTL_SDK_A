...
//		flmode = SpicOneBitMode;
	...
	uint8_t curclk = (HAL_READ32(SYSTEM_CTRL_BASE, REG_SYS_CLK_CTRL1)
...
int flash_stream_read(flash_t *obj, uint32_t address, uint32_t len,
	...
	if ((uint32_t) pbuf & 0x03) {
		...
		while (len >= 4) {
			*((uint32_t *) pbuf) = HAL_READ32(SPI_FLASH_BASE, address);
			pbuf += 4;
			address += 4;
			len -= 4;
		}
...
int flash_stream_write(flash_t *obj, uint32_t address, uint32_t len,
	...
	if ((uint32_t) pbuf & 0x03) {
		while (len >= 4) {
			write_word = (uint32_t) (*pbuf) | ((uint32_t) (*(pbuf + 1)) << 8)
		...
		while (len >= 4) {
			//Write word
			HAL_WRITE32(SPI_FLASH_BASE, address, (uint32_t)*((uint32_t *)pbuf));
			// Wait spic busy done
			SpicWaitBusyDoneRtl8195A();
			// Wait flash busy done (wip=0)
			if (flashtype == FLASH_MICRON) {
 ...
 * Read Flash OTP data
 */
extern void SpicTxCmdWithDataRtl8195A(uint8_t cmd, uint8_t DataPhaseLen, uint8_t* pData, SPIC_INIT_PARA SpicInitPara);
