...
int nfc_cache_write(nfctag_t *obj, uint32_t *tbuf, unsigned int spage, unsigned int pg_num)
{
    ...
    while (remain_pg > 0) {
        if (remain_pg >= 4) {            
            A2NWriteCatch (&obj->NFCAdapter, (spage+pg_offset), 4, (uint32_t*)(&tbuf[pg_offset]));
            remain_pg -= 4;
            pg_offset += 4;
        }
        else {
            for(i=0;i<remain_pg;i++) {
                A2NWriteCatch (&obj->NFCAdapter, (spage+pg_offset), 1, (uint32_t*)(&tbuf[pg_offset]));
                pg_offset++;
            }
            remain_pg = 0;
        }
    ...
    void *arg, unsigned int start_pg)
{
    ...
    A2NReadCatch(&(obj->NFCAdapter), (uint8_t)start_pg);
