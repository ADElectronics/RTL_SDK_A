...
const uint8_t Default_Port_PinDef[GPIO_PORT_NUM][GPIO_PORT_WIDTH+1] = {
...
extern const uint8_t GPIO_SWPORT_DR_TBL[];
extern const uint8_t GPIO_EXT_PORT_TBL[];

extern  void HAL_GPIO_Init(HAL_GPIO_PIN  *GPIO_Pin);
extern uint32_t HAL_GPIO_GetPinName(uint32_t chip_pin);
