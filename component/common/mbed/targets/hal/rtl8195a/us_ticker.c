...
void us_ticker_init(void) 
{
    ...
    Timer6Adapter.IrqHandle.Data = (uint32_t)NULL;
...
{
    HalTimerOp.HalTimerDis((uint32_t)Timer6Adapter.TimerId);
}

void us_ticker_clear_interrupt(void) 
{
    HalTimerOp.HalTimerIrqClear((uint32_t)Timer6Adapter.TimerId);
}
