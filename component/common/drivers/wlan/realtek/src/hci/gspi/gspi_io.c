...
uint8_t spi_read8(struct dvobj_priv *pdvobj, uint32_t addr, int32_t *err);
uint16_t spi_read16(struct dvobj_priv *pdvobj, uint32_t addr, int32_t *err);
uint32_t spi_read32(struct dvobj_priv *pdvobj, uint32_t addr, int32_t *err);
int32_t spi_write8(struct dvobj_priv *pdvobj, uint32_t addr, uint8_t buf, int32_t *err);
int32_t spi_write16(struct dvobj_priv *pdvobj, uint32_t addr, uint16_t buf, int32_t *err);
int32_t spi_write32(struct dvobj_priv *pdvobj, uint32_t addr, uint32_t buf, int32_t *err);

static uint32_t rtw_spi_transfer(
...
static int addr_convert(uint32_t addr)
{
	uint32_t domain_id = 0 ;
	uint32_t temp_addr = addr&0xffff0000;
 ...
 */
static uint32_t hwaddr2txfifo(uint32_t addr)
{
	uint32_t fifo_domain_id;
	switch (addr)
	{
...
static uint32_t buf_endian_reverse(uint32_t src)
{
...
uint8_t spi_query_status_info(struct dvobj_priv *pdvobj)
{
...
static void spi_get_status_info(struct dvobj_priv *pdvobj, unsigned char *status)
{
	...
	uint32_t local_status[2];
	uint8_t *pstatus = (uint8_t*)(&local_status[0]);

	memcpy(pstatus, status, GSPI_STATUS_LEN); 
#else
	uint8_t *pstatus = status;
#endif
...
static int spi_read_write_reg(struct dvobj_priv *pdvobj, int  write_flag, uint32_t addr, char * buf, int len, uint32_t eddien)
{
	int  fun = 1, domain_id = 0x0; //static
	unsigned int cmd = 0 ;
	int byte_en = 0 ;//,i = 0 ;
	int ret = 1;
	unsigned char status[8] = {0};
	unsigned int data_tmp = 0;
	uint32_t force_bigendian = eddien;

	uint32_t spi_buf[4] = {0};
	bool polled = TRUE;
	...
	if (!write_flag && (domain_id!= RX_RXFIFO_DOMAIN)) {
		...
		rtw_spi_transfer(pdvobj, polled, (uint8_t*)spi_buf, sizeof(spi_buf));

		rtw_memcpy(status, (uint8_t *) &spi_buf[1], sizeof(status));
		...
		*(uint32_t*)buf = read_data;
		...
		rtw_memcpy(status, (uint8_t *) &spi_buf[2], sizeof(status));
	}
...
static int spi_write8_endian(struct dvobj_priv *pdvobj, uint32_t addr, uint32_t buf, uint32_t big)
{
...
static int spi_read_rx_fifo(struct dvobj_priv *pdvobj, uint32_t addr, uint8_t *buf, uint32_t len, struct fifo_more_data *pmore_data)
{
	int fun = 1, domain_id = RX_RXFIFO_DOMAIN;
	unsigned int cmd = 0;
	unsigned char *status = buf + len;
	uint8_t *spi_buf = (uint8_t *) (buf - GSPI_CMD_LEN);
	int spi_buf_len = 0;
	bool polled = TRUE;
	bool use_alloc = FALSE;
	uint32_t max_skb_len = 0;
...
#ifdef CONFIG_MEMORY_ACCESS_ALIGNED
	memcpy(spi_buf, (uint8_t *)&cmd, sizeof(int));
#else
	*((uint32_t *) spi_buf) = cmd;
#endif

	rtw_spi_transfer(pdvobj, polled, (uint8_t *) spi_buf, GSPI_CMD_LEN + len + GSPI_STATUS_LEN);
...
static int spi_write_tx_fifo(struct dvobj_priv *pdvobj, uint32_t addr, uint8_t *buf, uint32_t len)
{
