...
int32_t gspi_dvobj_xmit_mgnt(_adapter * padapter, struct xmit_frame *pmgntframe)
{
...
//#include <skbuff.h>
int32_t gspi_dvobj_xmit_data(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	struct xmit_priv 	*pxmitpriv = &padapter->xmitpriv;
	_irqL irql;
	int32_t err;
...
static uint8_t gspi_alloc_irq(struct dvobj_priv *dvobj)
{
	PGSPI_DATA pgspi_data;
...
extern int pm_netdev_open(struct net_device *pnetdev,uint8_t bnormal);
int rtw_resume_process(_adapter *padapter)
{
	struct net_device *pnetdev;
	struct pwrctrl_priv *pwrpriv;
	uint8_t is_pwrlock_hold_by_caller;
	uint8_t is_directly_called_by_auto_resume;
	int ret = 0;
	uint32_t start_time = rtw_get_current_time();
