...
static inline uint32_t ffaddr2deviceId(struct dvobj_priv *pdvobj, uint32_t addr)
{
	return pdvobj->Queue2Pipe[addr];
}

static int32_t rtw_xmit_xmitbuf(_adapter * padapter, struct xmit_buf *pxmitbuf)
{
	...
	rtw_write_port(padapter, deviceId, pxmitbuf->len, (uint8_t*)pxmitbuf->pbuf);

	rtw_free_xmitbuf(pxmitpriv, pxmitbuf);
	return _SUCCESS;
}

int32_t sdio_dvobj_xmit_mgnt(_adapter * padapter, struct xmit_frame *pmgntframe)
{
...
int32_t sdio_dvobj_xmit_data(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	struct xmit_priv 	*pxmitpriv = &padapter->xmitpriv;
	_irqL irql;
	int32_t err;
