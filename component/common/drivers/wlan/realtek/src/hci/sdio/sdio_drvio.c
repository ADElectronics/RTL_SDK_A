...
void sdio_cmd52_read(ADAPTER *Adapter, uint32_t addr, uint32_t cnt, uint8_t *pdata, int *err)
{
	int i = 0;
...
void sdio_cmd52_write(ADAPTER *Adapter, uint32_t addr, uint32_t cnt, uint8_t *pdata, int *err)
{
	int i = 0;
...
uint8_t _sdio_read8(struct dvobj_priv *pdvobj, uint32_t addr, int32_t *err)
{
	uint8_t val;
...
uint16_t _sdio_read16(struct dvobj_priv *pdvobj, uint32_t addr, int32_t *err)
{
	uint8_t bMacPwrCtrlOn = _FALSE;
	uint16_t val;
	uint8_t cmd52_io = 0;
	ADAPTER *Adapter = pdvobj->if1;
...
#endif
	if (cmd52_io) {
		sdio_cmd52_read(Adapter, addr, 2, (uint8_t*)&val, err);
		val = le16_to_cpu(val);
		return val;
	}
...
uint32_t _sdio_read32(struct dvobj_priv *pdvobj, uint32_t addr, int32_t *err)
{
	uint8_t bMacPwrCtrlOn = _FALSE;
	uint32_t val;
	uint8_t cmd52_io = 0;
...
#endif
	if (cmd52_io) {
		sdio_cmd52_read(Adapter, addr, 4, (uint8_t*)&val, err);
		val = le32_to_cpu(val);
		return val;
	}
...
int32_t _sdio_write8(struct dvobj_priv *pdvobj, uint32_t addr, uint8_t val, int32_t *err)
{
	ADAPTER *Adapter = pdvobj->if1;

_func_enter_;

	sdio_write8(Adapter, addr, (uint32_t)val, err);
...
int32_t _sdio_write16(struct dvobj_priv *pdvobj, uint32_t addr, uint16_t val, int32_t *err)
{
	uint8_t bMacPwrCtrlOn = _FALSE;
	uint8_t cmd52_io = 0;
	ADAPTER *Adapter = pdvobj->if1;
...
_func_enter_;
	...
	if (cmd52_io) {
		val = cpu_to_le16(val);
		sdio_cmd52_write(Adapter, addr, 2, (uint8_t*)&val, err);
		return _SUCCESS;
	}

	sdio_write16(Adapter, addr, (uint32_t)val, err);
...
int32_t _sdio_write32(struct dvobj_priv *pdvobj, uint32_t addr, uint32_t val, int32_t *err)
{
	uint8_t bMacPwrCtrlOn = _FALSE;
	uint8_t cmd52_io = 0;
	ADAPTER *Adapter = pdvobj->if1;
...
_func_enter_;
	...
	if (cmd52_io) {
		val = cpu_to_le32(val);
		sdio_cmd52_write(Adapter, addr, 4, (uint8_t*)&val, err);
		return _SUCCESS;
	}
 ...
 *	_FAIL(0)		Fail
 */
static int _sdio_read_rx_fifo(struct dvobj_priv *pdvobj, uint32_t addr, uint8_t *mem, uint32_t cnt, struct fifo_more_data *pmore_data)
{
	ADAPTER *Adapter = pdvobj->if1;
...
static int _sdio_write_tx_fifo(struct dvobj_priv *pdvobj, uint32_t addr, uint8_t *mem, uint32_t cnt)
{
	uint8_t remain_len = 0;
	uint32_t w_sz = cnt;
	ADAPTER *Adapter = pdvobj->if1;
	...
	if (remain_len != 0)
		...
		if (1) {
			int i = 0;
			for(i = 0; i < w_sz; i += 4) {
				DBG_871X("_sdio_write_port[%d]: 0x%08x ", i,  *(uint32_t*)(mem + i));
			}
		}
#endif
