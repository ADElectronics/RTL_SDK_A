...
struct wifi_display_info{
	uint16_t					wfd_enable;			//	Eanble/Disable the WFD function.
	uint16_t					rtsp_ctrlport;		//	TCP port number at which the this WFD device listens for RTSP messages
	uint16_t					peer_rtsp_ctrlport;	//	TCP port number at which the peer WFD device listens for RTSP messages
...
struct tx_provdisc_req_info{
	uint16_t					wps_config_method_request;	//	Used when sending the provisioning request frame
	uint16_t					peer_channel_num[2];		//	The channel number which the receiver stands.
...
struct wifidirect_info{
	...
	uint16_t						device_password_id_for_nego;	//	The device password ID for group negotation
	...
	uint16_t						supported_wps_cm;			//	This field describes the WPS config method which this driver supported.
...
#ifdef CONFIG_CONCURRENT_MODE
	uint16_t						ext_listen_interval;	//	The interval to be available with legacy AP (ms)
	uint16_t						ext_listen_period;	//	The time period to be available for P2P listen state (ms)
#endif
	uint8_t						p2p_ps_enable;
	...
	uint32_t						noa_duration[P2P_MAX_NOA_NUM]; // Max duration for owner, preferred or min acceptable duration for client.
	uint32_t						noa_interval[P2P_MAX_NOA_NUM]; // Length of interval for owner, preferred or max acceptable interval of client.
	uint32_t						noa_start_time[P2P_MAX_NOA_NUM]; // schedule expressed in terms of the lower 4 bytes of the TSF timer.
#endif
};
#if 0
...
struct mlme_priv {
	...
	uint8_t to_roaming; // roaming trying times
	#endif

	uint8_t		*nic_hdl;		//can be removed
	...
	uint32_t scan_start_time; // used to evaluate the time spent in scanning
	...
	uint16_t num_sta_no_ht; //int num_sta_no_ht;

	/* Number of HT AP/stations 20 MHz */
	//int num_sta_ht_20mhz; 


	uint16_t num_FortyMHzIntolerant; //int num_FortyMHzIntolerant;
...
//	uint8_t *p2p_go_probe_resp_ie; //for GO	
	...
	uint8_t *p2p_assoc_rsp_ie;

	uint32_t p2p_beacon_ie_len;
	uint32_t p2p_probe_req_ie_len;
	uint32_t p2p_probe_resp_ie_len;
//	uint32_t p2p_go_probe_resp_ie_len; //for GO
	uint32_t p2p_assoc_req_ie_len;
	uint32_t p2p_assoc_rsp_ie_len;
#endif //CONFIG_P2P
#endif //CONFIG_WPS
	...
	//uint8_t bcn_buf[256];
	uint8_t bcn_buf[320];  //p2p go beacon size is about 272+32 bytes
#endif
...
#ifdef CONFIG_MULTICAST
	uint32_t multicast_list[MULTICAST_LIST_SIZE];
#endif
...
extern void rtw_joinbss_event_prehandle(_adapter *adapter, uint8_t *pbuf);
extern void rtw_survey_event_callback(_adapter *adapter, uint8_t *pbuf);
...
extern void rtw_atimdone_event_callback(_adapter *adapter, uint8_t *pbuf);
extern void rtw_cpwm_event_callback(_adapter *adapter, uint8_t *pbuf);
...
extern void rtw_free_network_queue(_adapter *adapter,uint8_t isfreeall);
...
extern uint16_t rtw_get_capability(WLAN_BSSID_EX *bss);
extern void rtw_update_scanned_network(_adapter *adapter, WLAN_BSSID_EX *target);
extern void rtw_disconnect_hdl_under_linked(_adapter* adapter, struct sta_info *psta, uint8_t free_assoc);
...
extern int rtw_restruct_sec_ie(_adapter *adapter,uint8_t *in_ie,uint8_t *out_ie,uint in_len);
extern int rtw_restruct_wmm_ie(_adapter *adapter, uint8_t *in_ie, uint8_t *out_ie, uint in_len, uint initial_out_len);
...
void rtw_set_scan_deny(struct mlme_priv *mlmepriv, uint32_t ms);
#endif
...
extern void _rtw_free_network_queue(_adapter* padapter, uint8_t isfreeall);
...
void rtw_update_ht_cap(_adapter *padapter, uint8_t *pie, uint ie_len);
...
void rtw_proxim_send_packet(_adapter *padapter,uint8_t *pbuf,uint16_t len,uint8_t hw_rate);
