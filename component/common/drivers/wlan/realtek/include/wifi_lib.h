...
struct __attribute__((packed)) __attribute__((aligned(1))) rtk_sc {
	...
	uint8_t password[64];
	uint32_t ip_addr;
	uint8_t sync_pkt[9][6];
	uint8_t profile_pkt[256][6];
	uint32_t profile_pkt_len;
	uint8_t plain_buf[256];
	uint32_t plain_len;
	uint8_t key_buf[32];
	uint32_t key_len;
	uint8_t crypt_buf[256];
	uint32_t crypt_len;
	int32_t pattern_index;
	struct pattern_ops *pattern[5];
	uint8_t max_pattern_num;
	uint8_t pin[65];
	uint8_t default_pin[65];
	uint8_t have_pin;
	uint16_t device_type;
	uint8_t device_name[64];
	uint8_t bcast_crypt_buf[256];
};

struct pattern_ops;

typedef int32_t (*sc_check_pattern_call_back)(struct pattern_ops *, struct rtk_sc *);
typedef int32_t (*sc_get_cipher_info_call_back)(struct pattern_ops *, struct rtk_sc *);
typedef int32_t (*sc_generate_key_call_back)(struct pattern_ops *, struct rtk_sc *);
typedef int32_t (*sc_decode_profile_call_back)(struct pattern_ops *, struct rtk_sc *);
typedef int32_t (*sc_get_tlv_info_call_back)(struct pattern_ops *, struct rtk_sc *);
