...
typedef int32_t (*c2h_id_filter)(uint8_t *c2h_evt);

struct hal_ops {
	uint32_t	(*hal_power_on)(_adapter *padapter);
	uint32_t	(*hal_init)(_adapter *padapter);
	uint32_t	(*hal_deinit)(_adapter *padapter);

	void	(*free_hal_data)(_adapter *padapter);

	uint32_t	(*inirp_init)(_adapter *padapter);
	uint32_t	(*inirp_deinit)(_adapter *padapter);
	...
	void (*update_txdesc)(struct xmit_frame *pxmitframe, uint8_t *pbuf);
...
//pvvx	void	(*clear_interrupt)(_adapter *padapter); // нету в SDK !
	...
	void (*hal_set_wowlan_fw)(_adapter *adapter, uint8_t sleep);
#endif //CONFIG_WOWLAN
...
#endif
	void	(*SetHwRegHandler)(_adapter *padapter, uint8_t	variable,uint8_t* val);
	void	(*GetHwRegHandler)(_adapter *padapter, uint8_t	variable,uint8_t* val);
	...
	void	(*UpdateRAMaskHandler)(_adapter *padapter, uint32_t mac_id, uint8_t rssi_level);
#if 0
	void	(*SetBeaconRelatedRegistersHandler)(_adapter *padapter);
#endif

	void	(*Add_RateATid)(_adapter *padapter, uint32_t bitmap, uint8_t *arg, uint8_t rssi_level);
...
#endif
#if 0
	uint8_t	(*interface_ps_func)(_adapter *padapter,HAL_INTF_PS_FUNC efunc_id, uint8_t* val);
#endif
	...
	uint32_t	(*read_bbreg)(_adapter *padapter, uint32_t RegAddr, uint32_t BitMask);
	void	(*write_bbreg)(_adapter *padapter, uint32_t RegAddr, uint32_t BitMask, uint32_t Data);
	uint32_t	(*read_rfreg)(_adapter *padapter, uint32_t eRFPath, uint32_t RegAddr, uint32_t BitMask);
	void	(*write_rfreg)(_adapter *padapter, uint32_t eRFPath, uint32_t RegAddr, uint32_t BitMask, uint32_t Data);
	...
	void (*ReadEFuse)(_adapter *padapter, uint8_t efuseType, uint16_t _offset, uint16_t _size_byte, uint8_t *pbuf, BOOLEAN bPseudoTest);
	void (*EFUSEGetEfuseDefinition)(_adapter *padapter, uint8_t efuseType, uint8_t type, void *pOut, BOOLEAN bPseudoTest);
	uint16_t	(*EfuseGetCurrentSize)(_adapter *padapter, uint8_t efuseType, BOOLEAN bPseudoTest);
#if 0
...
#ifdef CONFIG_IOL
	int (*IOL_exec_cmds_sync)(_adapter *padapter, struct xmit_frame *xmit_frame, uint32_t max_wating_ms);
#endif
...
#endif	
	int32_t (*fill_h2c_cmd)(PADAPTER, uint8_t ElementID, uint32_t CmdLen, uint8_t *pCmdBuffer);
	void (*fill_fake_txdesc)(PADAPTER, uint8_t *pDesc, uint32_t BufferLen,
			uint8_t IsPsPoll, uint8_t IsBTQosNull, uint8_t bDataFrame);
	uint8_t (*hal_get_tx_buff_rsvd_page_num)(_adapter *adapter, bool wowlan);
};
...
#if 0
uint8_t	rtw_hal_intf_ps_func(_adapter *padapter,HAL_INTF_PS_FUNC efunc_id, uint8_t* val);
#endif
...
void	rtw_hal_add_ra_tid(_adapter *padapter, uint32_t bitmap, uint8_t *arg, uint8_t rssi_level);
...
#if 1
void rtw_hal_update_txdesc(_adapter  *padapter, struct xmit_frame *pxmitframe, uint8_t *pbuf);
#endif
int32_t rtw_hal_fill_h2c_cmd(PADAPTER padapter, uint8_t ElementID, uint32_t CmdLen, uint8_t *pCmdBuffer);

void rtw_hal_fill_fake_txdesc(_adapter* padapter, uint8_t* pDesc, uint32_t BufferLen,
		uint8_t IsPsPoll, uint8_t IsBTQosNull, uint8_t bDataFrame);

uint8_t rtw_hal_get_txbuff_rsvd_page_num(_adapter *padapter, bool wowlan);


void rtw_hal_set_wowlan_fw(_adapter *padapter, uint8_t sleep);


c2h_id_filter rtw_hal_c2h_id_filter_ccx(_adapter *padapter);

int32_t rtw_hal_c2h_handler(_adapter *padapter, uint8_t *c2h_evt);
...
int32_t	rtw_hal_recv_tasklet(_adapter *padapter);
...
uint32_t rtl8711bb_set_hal_ops(_adapter * padapter);
