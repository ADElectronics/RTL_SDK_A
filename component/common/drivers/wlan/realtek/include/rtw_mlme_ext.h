...
extern const uint8_t WMM_OUI[];
extern const uint8_t WPS_OUI[];
extern const uint8_t WFD_OUI[];
extern const uint8_t P2P_OUI[];

//extern const unsigned char WMM_INFO_OUI[];
extern const uint8_t	WMM_PARA_OUI[];
...
struct mlme_ext_info
{
	...
	uint32_t	link_count;
	uint32_t	auth_seq;
	uint32_t	auth_algo;	// 802.11 auth, could be open, shared, auto
	uint32_t	authModeToggle;
	uint32_t	enc_algo;//encrypt algorithm;
	uint32_t	key_index;	// this is only valid for legendary wep, 0~3 for key id.
	uint32_t	iv;
	uint8_t	chg_txt[128];
	uint16_t	aid;
...
typedef struct _RT_CHANNEL_INFO
{
	...
	//uint16_t				ScanPeriod;		// Listen time in millisecond in this channel.
	//int32_t				MaxTxPwrDbm;	// Max allowed tx power.
	//uint32_t				ExInfo;			// Extended Information for this channel.
...
struct mlme_ext_priv
{
	...
	uint8_t	cur_channel;
	uint8_t	cur_bwmode;
	uint8_t	cur_ch_offset;//PRIME_CHNL_OFFSET
	uint8_t	cur_wireless_mode;	// NETWORK_TYPE
	uint8_t	max_chan_nums;
	RT_CHANNEL_INFO		channel_set[MAX_CHANNEL_NUM];
#ifdef CONFIG_P2P_NEW
	RT_CHANNEL_INFO		social_channel_set[4];
	RT_CHANNEL_INFO		special_channel[2];
	uint8_t			special_mac[ETH_ALEN];
	uint16_t		scan_mode;
	uint8_t		bremain_on_channel;
	_timer remainon_timer;
#endif
	uint8_t	basicrate[NumRates];
	uint8_t	datarate[NumRates];
	...
	uint8_t	retry; //retry for issue probereq
	...
	uint16_t reconnect_timeout; // the unit is second
	uint8_t saved_alg;
	uint8_t saved_essid[32+1];
	uint8_t saved_key[32];
	uint16_t saved_key_len;
	uint8_t saved_key_idx;
	uint8_t saved_wpa_passphrase[IW_PASSPHRASE_MAX_SIZE + 1];
...
uint8_t judge_network_type(_adapter *padapter, unsigned char *rate, int ratelen);
void get_rate_set(_adapter *padapter, unsigned char *pbssrate, int *bssrate_len);
void UpdateBrateTbl(_adapter *padapter,uint8_t *mBratesOS);
...
//void Set_NETYPE1_MSR(_adapter *padapter, uint8_t type);
//void Set_NETYPE0_MSR(_adapter *padapter, uint8_t type);
void Set_MSR(_adapter *padapter, uint8_t type);

uint8_t set_opmode(_adapter  *padapter, NDIS_802_11_NETWORK_INFRASTRUCTURE networktype);
...
uint8_t* get_my_bssid(WLAN_BSSID_EX *pnetwork);
...
int update_sta_support_rate(_adapter *padapter, uint8_t* pvar_ie, uint var_ie_len, int cam_idx);
...
extern uint8_t set_tx_beacon_cmd(_adapter*padapter);
...
void issue_p2p_provision_request( _adapter *padapter, uint8_t* pssid, uint8_t ussidlen, uint8_t* pdev_raddr);
void issue_p2p_GO_request(_adapter *padapter, uint8_t* raddr);
void issue_probereq_p2p(_adapter *padapter);
void issue_p2p_invitation_response(_adapter *padapter, uint8_t* raddr, uint8_t dialogToken, uint8_t success);
void issue_p2p_invitation_request(_adapter *padapter, uint8_t* raddr );
#endif //CONFIG_P2P
...
void issue_probereq(_adapter *padapter, NDIS_802_11_SSID *pssid, uint8_t blnbc);
void issue_nulldata(_adapter *padapter, unsigned int power_mode);
void issue_qos_nulldata(_adapter *padapter, unsigned char *da, uint16_t tid);
void issue_deauth(_adapter *padapter, unsigned char *da, uint32_t reason);
void issue_action_BA(_adapter *padapter, unsigned char *raddr, unsigned char action, unsigned short status);
unsigned int send_delba(_adapter *padapter, uint8_t initiator, uint8_t *addr);
...
extern void process_addba_req(_adapter *padapter, uint8_t *paddba_req, uint8_t *addr);
#endif
...
#ifdef CONFIG_CONCURRENT_MODE
 sint check_buddy_mlmeinfo_state(_adapter *padapter, uint32_t state);
...
uint8_t read_macreg_hdl(_adapter *padapter, uint8_t *pbuf);
uint8_t write_macreg_hdl(_adapter *padapter, uint8_t *pbuf);
uint8_t read_bbreg_hdl(_adapter *padapter, uint8_t *pbuf);
uint8_t write_bbreg_hdl(_adapter *padapter, uint8_t *pbuf);
uint8_t read_rfreg_hdl(_adapter *padapter, uint8_t *pbuf);
uint8_t write_rfreg_hdl(_adapter *padapter, uint8_t *pbuf);

#endif	//#if 0

uint8_t NULL_hdl(_adapter *padapter, uint8_t *pbuf);
uint8_t join_cmd_hdl(_adapter *padapter, uint8_t *pbuf);
uint8_t disconnect_hdl(_adapter *padapter, uint8_t *pbuf);
uint8_t createbss_hdl(_adapter *padapter, uint8_t *pbuf);
uint8_t setopmode_hdl(_adapter *padapter, uint8_t *pbuf);
uint8_t sitesurvey_cmd_hdl(_adapter *padapter, uint8_t *pbuf);	
uint8_t setauth_hdl(_adapter *padapter, uint8_t *pbuf);
uint8_t setkey_hdl(_adapter *padapter, uint8_t *pbuf);
uint8_t set_stakey_hdl(_adapter *padapter, uint8_t *pbuf);
uint8_t set_assocsta_hdl(_adapter *padapter, uint8_t *pbuf);
uint8_t del_assocsta_hdl(_adapter *padapter, uint8_t *pbuf);
uint8_t add_ba_hdl(_adapter *padapter, unsigned char *pbuf);
...
uint8_t set_csa_hdl(_adapter *padapter, unsigned char *pbuf);	//Kurt: Handling DFS channel switch announcement ie.
...
void rtw_dummy_event_callback(_adapter *adapter , uint8_t *pbuf);
void rtw_fwdbg_event_callback(_adapter *adapter , uint8_t *pbuf);
