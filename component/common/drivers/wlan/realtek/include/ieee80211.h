...
//extern uint16_t RTW_WPA_VERSION ;
...
//tern uint8_t RSN_CIPHER_SUITE_WRAP[];
 ...
 *
	...
	struct {
		uint16_t association_id:14, reserved:2;
	} __attribute__ ((packed));
...
#define MAX_RATES_LENGTH                  ((uint8_t)12)
#define MAX_RATES_EX_LENGTH               ((uint8_t)16)
...
int ieee80211_get_hdrlen(uint16_t fc);
...
int rtw_get_wapi_ie(uint8_t *in_ie,uint in_len,uint8_t *wapi_ie,uint16_t *wapi_len);
...
uint8_t *rtw_get_p2p_ie(uint8_t *in_ie, uint in_len, uint8_t *p2p_ie, uint *p2p_ielen);
...
uint16_t rtw_mcs_rate(uint8_t rf_type, uint8_t bw_40MHz, uint8_t short_GI_20, uint8_t short_GI_40, unsigned char * MCS_rate);
#endif /* __IEEE80211_H */
