...
typedef struct _TxPowerInfo24G{
	...
	uint8_t IndexBW40_Base[MAX_RF_PATH][MAX_CHNL_GROUP_24G];
	//If only one tx, only BW20 and OFDM are used.
	int8_t OFDM_Diff[MAX_RF_PATH][MAX_TX_COUNT];
	int8_t BW20_Diff[MAX_RF_PATH][MAX_TX_COUNT];
#if !defined(NOT_SUPPORT_RF_MULTIPATH)
	int8_t CCK_Diff[MAX_RF_PATH][MAX_TX_COUNT];	
	int8_t BW40_Diff[MAX_RF_PATH][MAX_TX_COUNT];
#endif
