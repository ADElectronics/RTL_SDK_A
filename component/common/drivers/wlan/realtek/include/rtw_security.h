...
{
	uint32_t	  dot11AuthAlgrthm;		// 802.11 auth, could be open, shared, 8021x and authswitch 
	uint32_t	  dot11PrivacyAlgrthm;	// This specify the privacy for shared auth. algorithm.

	/* WEP */
	uint32_t	  dot11PrivacyKeyIndex;	// this is only valid for legendary wep, 0~3 for key id. (tx key index)
	union Keytype dot11DefKey[4];			// this is only valid for def. key	
	uint32_t 	dot11DefKeylen[4];

	uint32_t dot118021XGrpPrivacy;	// This specify the privacy algthm. used for Grp key 
	uint32_t	dot118021XGrpKeyid;		// key id used for Grp Key ( tx key index)
	...
	uint32_t ndisauthtype;	// NDIS_802_11_AUTHENTICATION_MODE
	uint32_t ndisencryptstatus;	// NDIS_802_11_ENCRYPTION_STATUS
...
//TODO
	...
	uint8_t					PMKIDIndex;
	//uint32_t				PMKIDCount;						// Added by Annie, 2006-10-13.
...
#endif
	uint8_t wpa_passphrase[IW_PASSPHRASE_MAX_SIZE + 1];
#endif
#ifdef CONFIG_WPS
	uint8_t	wps_phase;
#endif
...
uint32_t	rtw_init_sec_priv(_adapter *padapter);
