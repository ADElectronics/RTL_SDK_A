...
typedef struct _LED_871x{
	...
	uint8_t					bLedOn; // true if LED is ON, false if LED is OFF.

	uint8_t					bLedBlinkInProgress; // true if it is blinking, false o.w..

	uint8_t					bLedWPSBlinkInProgress;

	uint32_t					BlinkTimes; // Number of times to toggle led state for blinking.
	...
	uint8_t					bLedNoLinkBlinkInProgress;
	uint8_t					bLedLinkBlinkInProgress;
	uint8_t					bLedStartToLinkBlinkInProgress;
	...
	uint8_t					bLedSlowBlinkInProgress;//added by vivi, for led new mode
#endif
