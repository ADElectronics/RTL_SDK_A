...
#endif
	uint8_t *ifname[2] = {WLAN0_NAME,WLAN1_NAME};
...
static void cmd_wifi_scan_with_ssid(int argc, char **argv)
{
	...
	if(argc == 3 && argv[1] && argv[2]){
		...
		for(i = 3; i <= argc -1 ; i++){
			*(channel_list + i - 3) = (uint8_t)atoi(argv[i]);	
...
{
	CmdDumpWord(argc-1, (uint8_t**)(argv+1));
}
static void cmd_edit_reg(int argc, char **argv)
{
	CmdWriteWord(argc-1, (uint8_t**)(argv+1));
}
#endif
...
static void cmd_debug(int argc, char **argv)
{
	if(strcmp(argv[1], "ready_trx") == 0) {
		printf("%d\n", wifi_is_ready_to_transceive((rtw_interface_t)rtw_atoi((uint8_t*)argv[2])));
	} else if(strcmp(argv[1], "is_up") == 0) {
		printf("%d\n", wifi_is_up((rtw_interface_t)rtw_atoi((uint8_t*)argv[2])));
		...
		printf("%d\n", wifi_set_txpower(rtw_atoi((uint8_t*)argv[2])));
#endif		
...
void WlanNormal( IN  uint16_t argc, IN  uint8_t  *argv[])
{
	uint8_t i, j= 0;
	uint8_t*	pbuf = (uint8_t*)uart_buf;
	extern xSemaphoreHandle	uart_rx_interrupt_sema;
