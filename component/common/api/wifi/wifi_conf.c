...
void restore_wifi_info_to_flash() {
	...
	if (data_to_flash && p_write_reconnect_ptr) {
		...
		p_write_reconnect_ptr((uint8_t *) data_to_flash,
				sizeof(struct wlan_fast_reconnect));
...
int wifi_disconnect(void) {
	...
	const uint8_t null_bssid[ETH_ALEN + 2] = { 0, 0, 0, 0, 0, 1, 0, 0 };
...
int wifi_get_drv_ability(uint32_t *ability) {
	return wext_get_drv_ability(WLAN0_NAME, (uint32_t *)ability);
}
...
int wifi_get_channel(int *channel) {
	return wext_get_channel(WLAN0_NAME, (uint8_t*) channel);
}
...
error1_with_result_ptr:
	rtw_free((uint8_t*)scan_result_handler_ptr.pap_details);
	scan_result_handler_ptr.pap_details = NULL;

error2_with_result_ptr:
	rtw_free((uint8_t*)scan_result_handler_ptr.ap_details);
...
//----------------------------------------------------------------------------//
int wifi_set_pscan_chan(uint8_t * channel_list, uint8_t * pscan_config, uint8_t length) {
	if (channel_list)
 ...
 *
 * uint8_t test_1[] = {221, 2, 2, 2};
 * uint8_t test_2[] = {221, 2, 1, 1};
 * cus_ie buf[2] = {{test_1, PROBE_REQ},
 *		 {test_2, PROBE_RSP | BEACON}};
 * uint8_t buf_test2[] = {221, 2, 1, 3} ;
...
extern int promisc_add_packet_filter(uint8_t filter_id,
...
extern int wext_set_ch_deauth(const char *ifname, uint8_t enable);
int wifi_set_ch_deauth(uint8_t enable) {
