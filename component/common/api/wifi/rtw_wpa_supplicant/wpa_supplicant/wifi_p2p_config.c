...
int wifi_start_p2p_go(char *ssid, char *passphrase, uint8_t channel)
{
...
#if 1	
	uint32_t r = 0;
	os_get_random((uint8_t *) &r, sizeof(r));
...
{
	uint8_t *passphrase = "12345678";
	uint8_t channel = 6;	// 1, 6, 11
	...
	const uint8_t pri_dev_type[8] = {0x00,0x0A,0x00,0x50,0xF2,0x04,0x00,0x01};	// category ID:0x00,0x0A; sub category ID:0x00,0x01
	uint8_t res[P2P_GO_NEGO_RESULT_SIZE];
	uint16_t config_methods = WPS_CONFIG_DISPLAY | WPS_CONFIG_KEYPAD | WPS_CONFIG_PUSHBUTTON;
...
void cmd_p2p_listen(int argc, char **argv)
{
	...
	if(argc == 2){
		timeout = os_atoi((uint8_t*)argv[1]);
		printf("\r\n%s(): timeout=%d\n", __func__, timeout);
		if(timeout > 3600)
			timeout = 3600;
	}
...
void cmd_p2p_connect(int argc, char **argv)
{
	...
	char *pin = NULL;
	uint8_t dest[ETH_ALEN] = {0x44, 0x6d, 0x57, 0xd7, 0xce, 0x41};
	uint8_t res[P2P_GO_NEGO_RESULT_SIZE];
	int ret = 0;
...
#else //For test
	uint8_t dest1[ETH_ALEN] = {0xea, 0x92, 0xa4, 0x9b, 0x61, 0xd6};  //NEXUS 4
	//uint8_t dest1[ETH_ALEN] = {0x0e, 0x37, 0xdc, 0xfc, 0xc4, 0x12}; //HUAWEI U9508_c001
	//uint8_t dest1[ETH_ALEN] = {0x42, 0xcb, 0xa8, 0xd3, 0x2c, 0x50}; //HUAWEI G610-T00
