...
static _adapter * get_padapter(const char *ifname) {
	if(ifname[4] == '0') {
...
int wext_set_bssid(const char *ifname, const uint8_t *bssid) {
	struct iwreq iwr;
	memset(&iwr, 0, sizeof(iwr));
...
int wext_set_key_ext(const char *ifname, uint16_t alg, const uint8_t *addr,
		int key_idx, int set_tx, const uint8_t *seq, uint16_t seq_len, uint8_t *key,
		uint16_t key_len) {
	struct iwreq iwr;
	int ret = -1;
...
int wext_get_enc_ext(const char *ifname, uint16_t *alg, uint8_t *key_idx,
		uint8_t *passphrase) {
	struct iwreq iwr;
	int ret = -1;
	struct iw_encode_ext *ext;
	...
	if (ext != NULL) {
		...
		if (ret >= 0) {
			*alg = ext->alg;
			if (key_idx)
				*key_idx = (uint8_t ) iwr.u.encoding.flags;
			if (passphrase)
...
int wext_set_passphrase(const char *ifname, const uint8_t *passphrase,
...
int wext_enable_powersave(const char *ifname, uint8_t ips_mode, uint8_t lps_mode) {
#if USE_WIFI_ADAPTER
	_adapter * pad = get_padapter(ifname);
	rtw_result_t ret = RTW_ERROR;
	if(pad) {
...
int wext_set_tdma_param(const char *ifname, uint8_t slot_period,
		uint8_t rfon_period_len_1, uint8_t rfon_period_len_2, uint8_t rfon_period_len_3) {
#if USE_WIFI_ADAPTER
	_adapter * pad = get_padapter(ifname);
	rtw_result_t ret = RTW_ERROR;
	if(pad) {
...
int wext_set_tos_value(const char *ifname, uint8_t *tos_value) {
	struct iwreq iwr;
	uint8_t para[sizeof("set_tos_value") + 4];
...
int wext_get_tx_power(const char *ifname, uint8_t *poweridx) {
	...
	if (para != NULL) {
		...
		ret = iw_ioctl(ifname, SIOCDEVPRIVATE, &iwr);
		if (ret >= 0)
			memcpy(poweridx, (uint8_t *) (iwr.u.data.pointer), 20);
		vPortFree(para);
	}
#if	CONFIG_DEBUG_LOG > 3
	else {
...
int wext_set_pscan_channel(const char *ifname,
	...
	if (para != NULL) {
		...
		*(para + 12) = length;
		for (i = 0; i < length; i++) {
			*(para + 13 + i) = *(ch + i);
			*((uint16_t *) (para + 13 + length + i)) = *(pscan_config + i);
		}
...
int wext_set_autoreconnect(const char *ifname, uint8_t mode, uint8_t retyr_times,
		uint16_t timeout) {
	struct iwreq iwr;
	uint8_t para[sizeof("SetAutoRecnt") + 4];
...
int wext_get_autoreconnect(const char *ifname, uint8_t *mode) {
	struct iwreq iwr;
	uint8_t para[sizeof("GetAutoRecnt") + 1];
	...
	if (ret >= 0)
		*mode = *(uint8_t *) (iwr.u.data.pointer);
	return ret;
}

int wext_get_drv_ability(const char *ifname, uint32_t *ability) {
	int ret = -1;
...
int wext_add_custom_ie(const char *ifname, void *cus_ie, int ie_num) {
	...
	if (ie_num <= 0 || !cus_ie) {
		...
		if (para != NULL) {
			...
			//addr length
			*(uint32_t *) (para + cmd_len) = (uint32_t ) cus_ie; //ie addr
			//ie_num
			*(uint32_t *) (para + cmd_len + 4) = ie_num; //num of ie
...
int wext_update_custom_ie(const char *ifname, void * cus_ie, int ie_index) {
	...
	if (ie_index <= 0 || !cus_ie) {
		...
		if (para != NULL) {
			...
			*(uint32_t *) (para + cmd_len) = (uint32_t ) cus_ie; //ie addr
			//ie_index
			*(uint32_t *) (para + cmd_len + 4) = ie_index; //num of ie
...
int wext_set_forwarding(const char *ifname, char flg) {
	struct iwreq iwr;
	uint8_t para[sizeof("forwarding_set") + 1];
...
int wext_set_ch_deauth(const char *ifname, uint8_t enable) {
	char buf[16];
	memset(buf, 0, 16);
...
int wext_set_adaptivity_th_l2h_ini(uint8_t l2h_threshold) {
	...
	rtw_adaptivity_th_l2h_ini = (int8_t ) l2h_threshold;
	return 0;
}
