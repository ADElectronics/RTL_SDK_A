...
#ifdef NOT_USE_CALLS
static int wlan_init_done_callback(void) {
...
static uint8 chk_ap_netif_num(void)
{
...
/*static _adapter * get_padaptern(int num) {
...
static int _wext_cmp_ssid(int adapter_num, unsigned char *ssid)
{
	_adapter * pad = get_padaptern(adapter_num);
	int ret = 0;
...
static rtw_result_t _wext_get_mode(int adapter_num, int *mode) {
	_adapter * pad =	get_padaptern(adapter_num);
	rtw_result_t ret = RTW_ERROR;
	if(pad) {
...
static rtw_result_t _wext_get_channel(int adapter_num, uint8 *ch)
{
	_adapter * pad = get_padaptern(adapter_num);
	rtw_result_t ret = RTW_ERROR;
	if(pad) {
...
static rtw_result_t wifi_run_ap(void) {
...
static rtw_result_t StartStDHCPClient(void)
{
...
static void wifi_autoreconnect_hdl_(rtw_security_t security_type, char *ssid,
...
static void st_set_autoreconnect(uint8 mode, uint8 count, uint16 timeout) {
...
static rtw_result_t wifi_run_st(void) {
...
static int _wifi_on(rtw_mode_t mode) {
	int ret = 0;
/*
