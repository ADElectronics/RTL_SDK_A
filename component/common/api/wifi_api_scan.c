...
/* --------  WiFi Scan Start ------------------------- */
static int _wifi_scan_networks(rtw_scan_result_handler_t results_handler) {
	internal_scan_handler_t * pscan_rec = &scan_result_handler_ptr;
...
static int wext_set_pscan_channels(void) {
	struct iwreq iwr;
	int ret = -1;
	uint8_t *para;
	int i = 0;
	...
	if (para != NULL) {
		...
		*(para + 12) = SCAN_CHANNELS; // length
		for (i = 0; i < SCAN_CHANNELS; i++) {
			*(para + 13 + i) = i + 1;
			*((uint16_t *) (para + 13 + SCAN_CHANNELS + i)) = PSCAN_ENABLE;
		}
